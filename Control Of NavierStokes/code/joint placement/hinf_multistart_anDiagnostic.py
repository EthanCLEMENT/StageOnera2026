...
# =============================================================================
# COUPLED a/n DIAGNOSTICS: denominator progress s vs numerator progress t
# =============================================================================
...
#
# Run with:
#   RUN_COUPLED_AN_DIAGNOSTICS=1 PYTHONUNBUFFERED=1 MPI_TASK_GROUP_SIZE=1 \
#   srun --mpi=pmi2 -n 40 python -u your_file.py
#
# This tests whether the good region requires a and n to move together:
#   a(s) = a_open + s*(a_good - a_open)
#   n(t) = t*n_good
#   K    = 0
#
# Important points in the plots:
#   (s,t) = (0,0): open loop, a=a_open, n=0
#   (s,t) = (1,0): good denominator only, n=0; still open-loop response
#   (s,t) = (0,1): good numerator with open-loop denominator
#   (s,t) = (1,1): full target controller
# =============================================================================

import os as _os_can
import sys as _sys_can
import csv as _csv_can
import numpy as _np_can


def _can_safe_label(s):
    return str(s).replace(" ", "_").replace("/", "_").replace("-", "m").replace("+", "p")


def _can_default_a_r4():
    try:
        return _np_can.asarray(default_stable_a_center(4), dtype=_np_can.complex128).reshape(4)
    except Exception:
        return _np_can.array([4.4, 16.3, 18.6, 7.7], dtype=_np_can.complex128)


def _can_hardcoded_bt_lqg_r4():
    # Fallback coefficients from the r=4 balanced-truncated LQG companion export.
    # These are used only if lqg_r4_companion_warmstart.npz is not found.
    a = _np_can.array([
        57.640337 + 1493.5275j,
        5263.6031 + 8495.4118j,
        16368.3529 + 1760.5718j,
        1666.5588 - 1.7373j,
    ], dtype=_np_can.complex128)
    n = _np_can.array([
        -47176.7578 + 30216.0574j,
        -214440.8877 + 65873.9221j,
        -127112.9443 - 78063.9182j,
        -46559.8597 + 6333.7936j,
    ], dtype=_np_can.complex128)
    return {"label": "BT_LQG_r4_hardcoded", "a": a, "n": n, "K": 0.0, "source": "hard-coded fallback"}


def _can_try_load_npz(path, label):
    if not path or not _os_can.path.exists(path):
        return None
    data = _np_can.load(path, allow_pickle=True)
    if "a_complex" not in data.files or "n_complex" not in data.files:
        return None
    a = _np_can.asarray(data["a_complex"], dtype=_np_can.complex128).reshape(-1)
    n = _np_can.asarray(data["n_complex"], dtype=_np_can.complex128).reshape(-1)
    if a.size != 4 or n.size != 4:
        return None
    return {"label": label, "a": a, "n": n, "K": 0.0, "source": path}


def _can_load_target_controller():
    """
    Target controller for the coupled map.  Priority:
      1. COUPLED_TARGET_NPZ=/path/to/controller.npz
      2. HINF_GOOD_NPZ=/path/to/controller.npz
      3. BT_LQG_R4_NPZ or lqg_r4_companion_warmstart.npz
      4. hard-coded BT-LQG r=4 fallback

    The npz must contain keys a_complex and n_complex.
    """
    for env_name, label in [
        ("COUPLED_TARGET_NPZ", "target_npz"),
        ("HINF_GOOD_NPZ", "Hinf_good"),
        ("BT_LQG_R4_NPZ", "BT_LQG_r4"),
    ]:
        path = _os_can.environ.get(env_name, "")
        ctrl = _can_try_load_npz(path, label)
        if ctrl is not None:
            return ctrl

    ctrl = _can_try_load_npz("lqg_r4_companion_warmstart.npz", "BT_LQG_r4")
    if ctrl is not None:
        return ctrl

    return _can_hardcoded_bt_lqg_r4()


def _can_build_cldyn(xa_fixed, xs_fixed, r=4):
...
def _can_pack(cl, a, n, K=0.0):
    return cl.pack_theta(
        _np_can.asarray(a, dtype=_np_can.complex128).reshape(4),
        _np_can.asarray(n, dtype=_np_can.complex128).reshape(4),
        Kstat=float(K),
    )


def _can_eval_theta(cl, theta, wmin=1e-4, wmax=5e1, npos=30, nrefine=3, alpha_margin=1e-3):
    out = {
        "stable": False,
        "Hinf": _np_can.inf,
        "log10Hinf": _np_can.nan,
        "alpha": _np_can.nan,
        "c_stab": _np_can.nan,
        "omega_star": _np_can.nan,
        "eval_error": "",
    }
    try:
        ...
        w_star, g_star, _info = find_peak_candidate_dynamic(
            ...
            xatol=float(_os_can.environ.get("COUPLED_XATOL", "1e-7")),
            maxiter=int(_os_can.environ.get("COUPLED_REFINE_MAXITER", "60")),
        )
        out["Hinf"] = float(g_star)
        out["log10Hinf"] = float(_np_can.log10(max(float(g_star), 1e-300)))
...
def _can_write_csv(filename, rows, fieldnames):
    if IS_WORLD_ROOT:
        with open(filename, "w", newline="") as f:
            wr = _csv_can.DictWriter(f, fieldnames=fieldnames)
            wr.writeheader()
            for r in rows:
                wr.writerow({k: r.get(k, "") for k in fieldnames})


def _can_plot_grid(csv_name, prefix="coupled_an_slice", target_label="target"):
    if not IS_WORLD_ROOT:
        return
    try:
        import pandas as _pd_can
        import matplotlib.pyplot as _plt_can
        df = _pd_can.read_csv(csv_name)
        ss = _np_can.sort(df["s"].unique())
        tt = _np_can.sort(df["t"].unique())
        S, T = _np_can.meshgrid(ss, tt)

        def grid(col):
            Z = _np_can.full((len(tt), len(ss)), _np_can.nan)
            for _, row in df.iterrows():
                i = int(_np_can.where(tt == row["t"])[0][0])
                j = int(_np_can.where(ss == row["s"])[0][0])
                Z[i, j] = row[col]
            return Z
        ...
        plot_specs = [
            (Hlog, "log10Hinf", f"log10 Hinf: a(s), n(t), target={target_label}"),
            (alpha, "alpha", "spectral abscissa alpha"),
            (omega, "omega", "active peak frequency omega*"),
            (stable, "stable_mask", "stable mask: alpha + margin <= 0"),
        ]
        for Z, name, title in plot_specs:
            fig, ax = _plt_can.subplots(figsize=(7.6, 6.2))
            pcm = ax.pcolormesh(S, T, Z, shading="auto")
            # diagonal coupled path a and n together
            lo = max(float(ss.min()), float(tt.min()))
            hi = min(float(ss.max()), float(tt.max()))
            if hi > lo:
                diag = _np_can.linspace(lo, hi, 500)
                ax.plot(diag, diag, "w--", linewidth=1.3, alpha=0.85, label="coupled diagonal s=t")
            # key points
            pts = [(0, 0, "open"), (1, 0, "a_good only"), (0, 1, "n_good only"), (1, 1, "full good")]
            for x0, y0, lab in pts:
                if ss.min() <= x0 <= ss.max() and tt.min() <= y0 <= tt.max():
                    ax.scatter([x0], [y0], marker="x", s=80)
                    ax.text(x0, y0, " " + lab, va="bottom", fontsize=8)
            ax.set_xlabel("s: denominator progress  a = a_open + s(a_good-a_open)")
            ax.set_ylabel("t: numerator progress  n = t n_good")
            ax.set_title(title)
        ...
        print(f"Could not plot coupled a/n grid: {exc}", flush=True)


def _can_plot_paths(csv_name, prefix="coupled_an_paths"):
    if not IS_WORLD_ROOT:
        return
    try:
        import pandas as _pd_can
        ...
        fig, axs = _plt_can.subplots(3, 1, figsize=(8.2, 8.6), sharex=True)
        for p in paths:
            d = df[df["path"] == p].sort_values("tau")
            stable = d["stable"].astype(bool).to_numpy()
            H = d["Hinf"].to_numpy()
            H = _np_can.where(stable, H, _np_can.nan)
            axs[0].semilogy(d["tau"], H, marker="o", label=p)
            axs[1].plot(d["tau"], d["alpha"], marker="o", label=p)
            axs[2].plot(d["tau"], d["omega_star"], marker="o", label=p)
        axs[0].set_ylabel("Hinf")
        axs[1].set_ylabel("alpha")
        axs[2].set_ylabel("omega*")
        axs[2].set_xlabel("path parameter tau")
        axs[1].axhline(0.0, linestyle="--", linewidth=1)
        for ax in axs:
            ax.grid(True, which="both", alpha=0.35)
            ax.legend(fontsize=7)
        fig.suptitle("Coupled a/n diagnostic paths")
        fig.tight_layout()
        fig.savefig(f"{prefix}.png", dpi=180)
        _plt_can.close(fig)
    except Exception as exc:
        print(f"Could not plot coupled a/n paths: {exc}", flush=True)


def run_coupled_an_diagnostics():
    xa_fixed = float(_os_can.environ.get("COUPLED_XA", _os_can.environ.get("HINF_R4_NOK_XA", "-0.98")))
    xs_fixed = float(_os_can.environ.get("COUPLED_XS", _os_can.environ.get("HINF_R4_NOK_XS", "0.95")))
    n_grid = int(_os_can.environ.get("COUPLED_N", "13"))
    s_min = float(_os_can.environ.get("COUPLED_S_MIN", "-0.10"))
    s_max = float(_os_can.environ.get("COUPLED_S_MAX", "1.20"))
    t_min = float(_os_can.environ.get("COUPLED_T_MIN", "0.00"))
    t_max = float(_os_can.environ.get("COUPLED_T_MAX", "1.20"))
    wmin = float(_os_can.environ.get("COUPLED_WMIN", "1e-4"))
    wmax = float(_os_can.environ.get("COUPLED_WMAX", "5e1"))
    npos = int(_os_can.environ.get("COUPLED_NPOS", "30"))
    nrefine = int(_os_can.environ.get("COUPLED_NREFINE", "3"))
    alpha_margin = float(_os_can.environ.get("COUPLED_STAB_MARGIN", "1e-3"))

    _rprint("\n" + "#" * 100)
    _rprint("COUPLED a/n DIAGNOSTICS: a(s)=a_open+s(a_good-a_open), n(t)=t*n_good, K=0")
    _rprint(f"fixed placement xa/xs = {xa_fixed:+.10f}, {xs_fixed:+.10f}")
    _rprint(f"grid: {n_grid} x {n_grid}, s=[{s_min:g},{s_max:g}], t=[{t_min:g},{t_max:g}]")
    _rprint(f"Hinf search: wmin={wmin:g}, wmax={wmax:g}, npos={npos}, nrefine={nrefine}")
    _rprint("#" * 100 + "\n")

    cl, _b_vec, _c_vec = _can_build_cldyn(xa_fixed, xs_fixed, r=4)
    a_open = _can_default_a_r4()
    n_open = _np_can.zeros(4, dtype=_np_can.complex128)
    target = _can_load_target_controller()
    a_good = _np_can.asarray(target["a"], dtype=_np_can.complex128).reshape(4)
    n_good = _np_can.asarray(target["n"], dtype=_np_can.complex128).reshape(4)

    _rprint(f"target label/source: {target['label']} / {target['source']}")
    _rprint(f"||a_open||={_np_can.linalg.norm(a_open):.6e}, ||a_good||={_np_can.linalg.norm(a_good):.6e}")
    _rprint(f"||n_good||={_np_can.linalg.norm(n_good):.6e}")
    _rprint(f"a_open={a_open}")
    _rprint(f"a_good={a_good}")
    _rprint(f"n_good={n_good}\n")

    ss = _np_can.linspace(s_min, s_max, n_grid)
    tt = _np_can.linspace(t_min, t_max, n_grid)
    rows = []
    count = 0
    total = n_grid * n_grid
    for tv in tt:
        for sv in ss:
            count += 1
            a = a_open + float(sv) * (a_good - a_open)
            n = float(tv) * n_good
            theta = _can_pack(cl, a, n, K=0.0)
            rec = _can_eval_theta(cl, theta, wmin=wmin, wmax=wmax, npos=npos, nrefine=nrefine, alpha_margin=alpha_margin)
            rec.update({
                "s": float(sv),
                "t": float(tv),
                "norm_a": float(_np_can.linalg.norm(a)),
                "norm_n": float(_np_can.linalg.norm(n)),
                "target_label": target["label"],
            })
            rows.append(rec)
            _rprint(f"  grid {count:03d}/{total}: s={sv:+.3f}, t={tv:+.3f}, Hinf={rec['Hinf']:.4e}, alpha={rec['alpha']:+.3e}, omega={rec['omega_star']:+.3e}, stable={rec['stable']}")

    grid_fields = ["s", "t", "stable", "Hinf", "log10Hinf", "alpha", "c_stab", "omega_star", "norm_a", "norm_n", "target_label", "eval_error"]
    grid_csv = "coupled_an_slice_a_s_n_t.csv"
    _can_write_csv(grid_csv, rows, grid_fields)
    _can_plot_grid(grid_csv, prefix="coupled_an_slice", target_label=target["label"])

    # Diagnostic paths through the same map.
    path_defs = [
        # name, s(tau), t(tau)
        ("diagonal_coupled", lambda tau: tau, lambda tau: tau),
        ("n_only_open_den", lambda tau: 0.0, lambda tau: tau),
        ("n_ramp_good_den", lambda tau: 1.0, lambda tau: tau),
        ("a_ramp_good_num", lambda tau: tau, lambda tau: 1.0),
        ("a_only_zero_num", lambda tau: tau, lambda tau: 0.0),
    ]
    n_path = int(_os_can.environ.get("COUPLED_PATH_N", str(max(11, n_grid))))
    taus = _np_can.linspace(0.0, 1.0, n_path)
    path_rows = []
    _rprint(f"\n[path diagnostics] {len(path_defs)} path(s), {n_path} point(s) each")
    for pname, sf, tf in path_defs:
        for kk, tau in enumerate(taus):
            sv = float(sf(float(tau)))
            tv = float(tf(float(tau)))
            a = a_open + sv * (a_good - a_open)
            n = tv * n_good
            theta = _can_pack(cl, a, n, K=0.0)
            rec = _can_eval_theta(cl, theta, wmin=wmin, wmax=wmax, npos=npos, nrefine=nrefine, alpha_margin=alpha_margin)
            rec.update({
            ...
            _rprint(f"  {pname:>17s} {kk+1:02d}/{n_path}: tau={tau:.3f}, s={sv:.3f}, t={tv:.3f}, Hinf={rec['Hinf']:.4e}, alpha={rec['alpha']:+.3e}, stable={rec['stable']}")
    path_fields = ["path", "tau", "s", "t", "stable", "Hinf", "log10Hinf", "alpha", "c_stab", "omega_star", "norm_a", "norm_n", "eval_error"]
    path_csv = "coupled_an_paths.csv"
    _can_write_csv(path_csv, path_rows, path_fields)
    _can_plot_paths(path_csv, prefix="coupled_an_paths")

    # Print best stable grid point for quick reading.
    stable_rows = [r for r in rows if r.get("stable") and _np_can.isfinite(r.get("Hinf", _np_can.inf))]
    if stable_rows:
        best = min(stable_rows, key=lambda r: r["Hinf"])
        _rprint("\nBest stable grid point:")
        _rprint(f"  s={best['s']:+.6f}, t={best['t']:+.6f}")
        _rprint(f"  Hinf={best['Hinf']:.10e}, alpha={best['alpha']:+.10e}, omega={best['omega_star']:+.6e}")
    else:
        _rprint("\nNo stable finite-Hinf grid point found.")

    _rprint("\nCoupled a/n diagnostics complete.")
    _rprint("Wrote coupled_an_slice_*.png, coupled_an_slice_a_s_n_t.csv, coupled_an_paths.png, coupled_an_paths.csv\n")


if 1==1:
    run_coupled_an_diagnostics()
    _sys_can.exit(0)

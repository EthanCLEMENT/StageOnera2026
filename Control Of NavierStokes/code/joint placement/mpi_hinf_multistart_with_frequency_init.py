...
# =============================================================================
# FREQUENCY-DOMAIN INITIALIZATION: spectral target -> rational fit -> PyGRANSO
# =============================================================================
# This block is meant to be appended before the default true multistart execution.
# It implements an open-loop frequency-response initialization inspired by
# spectral-model / Nyquist-domain fixed-order design methods.  It does NOT use
# LQG.  It first constructs a desired loop response Ld(jw), converts it into a
# target controller response Kd(jw)=Ld(jw)/P(jw), fits a strictly proper r=4
# rational controller K(s)=N(s)/D(s), then uses that matched (a,n) pair to
# initialize the existing no-K PyGRANSO Hinf refinement.
# =============================================================================


def _fdinit_parse_float_list(env_name, default):
    txt = os.environ.get(env_name, default)
    return [float(x.strip()) for x in str(txt).split(',') if x.strip()]


def _fdinit_signed_log_grid(wmin=1e-3, wmax=5.0, npos=35, include_zero=False):
    ws_pos = np.logspace(np.log10(float(wmin)), np.log10(float(wmax)), int(npos))
    if include_zero:
        return np.concatenate([-ws_pos[::-1], [0.0], ws_pos]).astype(float)
    return np.concatenate([-ws_pos[::-1], ws_pos]).astype(float)


def _fdinit_extra_peak_grid(centers=(-0.65, -0.54, -0.42, 0.0), halfwidth=0.20, n_each=21):
    chunks = []
    for c0 in centers:
        chunks.append(np.linspace(float(c0)-float(halfwidth), float(c0)+float(halfwidth), int(n_each)))
    return np.unique(np.concatenate(chunks)).astype(float)


def _fdinit_build_fit_grid():
    wmin = float(os.environ.get("FDINIT_WMIN", "1e-3"))
    wmax = float(os.environ.get("FDINIT_WMAX", "8.0"))
    npos = int(os.environ.get("FDINIT_NPOS", "35"))
    ws = [_fdinit_signed_log_grid(wmin=wmin, wmax=wmax, npos=npos, include_zero=False)]
    if _env_flag("FDINIT_ADD_PEAK_GRID", True):
        centers = _fdinit_parse_float_list("FDINIT_PEAK_CENTERS", "-0.65,-0.54,-0.42,0.0")
        ws.append(_fdinit_extra_peak_grid(
            centers=centers,
            halfwidth=float(os.environ.get("FDINIT_PEAK_HALFWIDTH", "0.22")),
            n_each=int(os.environ.get("FDINIT_PEAK_NEACH", "21")),
        ))
    out = np.unique(np.concatenate(ws)).astype(float)
    # avoid exactly zero in the rational fit unless explicitly requested
    out = out[np.abs(out) > float(os.environ.get("FDINIT_W_EXCLUDE_ZERO", "1e-10"))]
    return out


def _fdinit_siso_P_at_omega(args):
    A_, M_, b_, c_, w_ = args
    S = build_S(A_, M_, float(w_))
    ksp = make_lu_ksp(S)
    q = S.createVecRight()
    ksp.solve(b_, q)
    # c^H q, consistent with y = c^H q in the dynamic class
    return complex(c_.dot(q))


def _fdinit_compute_siso_frequency_response(A_, M_, b_, c_, ws):
    items = [(A_, M_, b_, c_, float(w)) for w in ws]
    _rprint(f"[fd-init] computing SISO P(iw) on {len(items)} frequency points")
    vals = mpi_task_map(_fdinit_siso_P_at_omega, items, label="fdinit_siso_P_grid")
    P = np.asarray(vals, dtype=np.complex128)
    if IS_WORLD_ROOT:
        with open("fdinit_siso_P_grid.csv", "w", newline="") as f:
            wr = csv.writer(f)
            wr.writerow(["omega", "P_real", "P_imag", "absP"])
            for w, p in zip(ws, P):
                wr.writerow([float(w), float(np.real(p)), float(np.imag(p)), float(abs(p))])
    return P


def _fdinit_desired_loop(ws, amp=20.0, width=0.45, center=-0.54, baseline=0.0, kind="gaussian"):
    """Positive-feedback convention: S = 1/(1-L).

    We choose a negative-real desired loop Ld=-A(w).  Then, at frequencies where
    A is large, the scalar loop sensitivity magnitude would be roughly
        |S| = 1/(1+A),
    and the Nyquist curve is kept away from the positive-feedback critical point
    L=1.  This is only an initializer target; PyGRANSO later optimizes the true
    q->q Hinf norm.
    """
    ws = np.asarray(ws, dtype=float)
    amp = float(amp); width = max(float(width), 1e-12); center = float(center)
    baseline = float(baseline)
    if kind == "lorentz":
        shape = 1.0 / (1.0 + ((ws - center) / width) ** 2)
    elif kind == "two_peak":
        shape1 = np.exp(-0.5 * ((ws - center) / width) ** 2)
        shape2 = 0.45 * np.exp(-0.5 * ((ws + 0.05) / max(0.8*width, 1e-12)) ** 2)
        shape = np.maximum(shape1, shape2)
    elif kind == "band":
        shape = np.exp(-0.5 * ((ws - center) / width) ** 4)
    else:
        shape = np.exp(-0.5 * ((ws - center) / width) ** 2)
    Aloop = baseline + amp * shape
    return (-Aloop).astype(np.complex128)


def _fdinit_rational_fit_strictly_proper(ws, Kd, r=4, weights=None, sk_iters=4, stable_reflect=True):
    """Fit Kd(iw) with N(s)/D(s), D monic order r, deg N <= r-1.

    Coefficient convention returned:
        D(s) = s^r + a[r-1] s^(r-1) + ... + a[1] s + a[0]
        N(s) = b[r-1] s^(r-1) + ... + b[1] s + b[0]
    returned a,b are ascending powers [a0,...,a_{r-1}], [b0,...,b_{r-1}].
    """
    ws = np.asarray(ws, dtype=float).reshape(-1)
    s = 1j * ws
    H = np.asarray(Kd, dtype=np.complex128).reshape(-1)
    r = int(r)
    if weights is None:
        wgt = np.ones_like(ws, dtype=float)
    else:
        wgt = np.asarray(weights, dtype=float).reshape(-1)
        wgt = wgt / (np.max(wgt) + 1e-30)
        wgt = np.sqrt(np.maximum(wgt, 1e-12))

    # Unknown x = [b0..b_{r-1}, a0..a_{r-1}], complex.
    # Equation: sum_j b_j s^j - H * sum_j a_j s^j = H * s^r
    V = np.vstack([s ** j for j in range(r)]).T
    Aeq0 = np.hstack([V, -H[:, None] * V])
    beq0 = H * (s ** r)

    Dprev = np.ones_like(H, dtype=np.complex128)
    x = None
    for _ in range(max(1, int(sk_iters))):
        row_w = wgt / np.maximum(np.abs(Dprev), 1e-8)
        Aeq = Aeq0 * row_w[:, None]
        beq = beq0 * row_w
        x, *_ = np.linalg.lstsq(Aeq, beq, rcond=float(os.environ.get("FDINIT_LSTSQ_RCOND", "1e-10")))
        b = x[:r]
        a = x[r:]
        Dprev = s ** r + V @ a

    b = np.asarray(x[:r], dtype=np.complex128)
    a = np.asarray(x[r:], dtype=np.complex128)

    # Stabilize denominator by reflecting any RHP poles.  Then refit numerator
    # with the stabilized denominator fixed.
    if stable_reflect:
        poly_desc = np.concatenate([[1.0 + 0.0j], a[::-1]])
        poles = np.roots(poly_desc)
        poles_st = np.array([(-abs(np.real(p)) + 1j*np.imag(p)) if np.real(p) >= -1e-8 else p for p in poles], dtype=np.complex128)
        # keep poles from being too close to imaginary axis
        min_re = float(os.environ.get("FDINIT_MIN_POLE_REAL", "0.02"))
        poles_st = np.array([(-min_re + 1j*np.imag(p)) if np.real(p) > -min_re else p for p in poles_st], dtype=np.complex128)
        a = np.poly(poles_st)[1:][::-1].astype(np.complex128)
        D = s ** r + V @ a
        # fixed-denominator numerator LS: V b ≈ H D
        row_w = wgt / np.maximum(np.abs(D), 1e-8)
        b, *_ = np.linalg.lstsq(V * row_w[:, None], (H * D) * row_w, rcond=float(os.environ.get("FDINIT_LSTSQ_RCOND", "1e-10")))
        b = np.asarray(b, dtype=np.complex128)

    return a, b


def _fdinit_eval_rational(ws, a, b):
    ws = np.asarray(ws, dtype=float)
    s = 1j * ws
    a = np.asarray(a, dtype=np.complex128).reshape(-1)
...
def _fdinit_make_candidate_bank(ws, P, r=4):
    centers = _fdinit_parse_float_list("FDINIT_LD_CENTERS", "-0.65,-0.54,-0.42")
    amps = _fdinit_parse_float_list("FDINIT_LD_AMPS", "5,10,20,40,80")
    widths = _fdinit_parse_float_list("FDINIT_LD_WIDTHS", "0.18,0.30,0.45,0.70")
    baselines = _fdinit_parse_float_list("FDINIT_LD_BASELINES", "0,1")
    kinds = [x.strip() for x in os.environ.get("FDINIT_LD_KINDS", "gaussian,lorentz,two_peak").split(',') if x.strip()]
    epsP = float(os.environ.get("FDINIT_MIN_ABSP", "1e-10"))

    # weights emphasize frequencies where desired loop action is large and P is
    # not numerically invisible.
    cand = []
    cid = 0
    for kind in kinds:
            ...
            for amp in amps:
                for width in widths:
                    for baseline in baselines:
                        Ld = _fdinit_desired_loop(ws, amp=amp, width=width, center=center, baseline=baseline, kind=kind)
                        Kd = Ld / np.where(np.abs(P) > epsP, P, epsP * np.exp(1j*np.angle(P + 1e-30)))
                        # Avoid letting tiny plant response dominate the fit.
                        fit_weight = (np.abs(Ld) + 0.1) * np.minimum(1.0, np.abs(P) / (np.median(np.abs(P)) + 1e-30))
                        a, b = _fdinit_rational_fit_strictly_proper(
                            ws, Kd, r=r, weights=fit_weight,
                            sk_iters=int(os.environ.get("FDINIT_SK_ITERS", "4")),
                            stable_reflect=True,
                        )
                        Kfit = _fdinit_eval_rational(ws, a, b)
                        fit_rel = float(np.linalg.norm(np.sqrt(fit_weight)*(Kfit-Kd)) / (np.linalg.norm(np.sqrt(fit_weight)*Kd) + 1e-30))
                        cand.append({
                            "candidate_id": cid,
                            "kind": kind,
                            "center": float(center),
                            ...
                            "a": a,
                            "b": b,
                            # dynamic class uses n^H x.  If N coefficients are b_j,
                            # then n_j = conj(b_j).
                            "n": np.conjugate(b),
                            "fit_rel": fit_rel,
                        })
...
def _fdinit_evaluate_candidate(args):
    cl_dyn, cand, wmin, wmax, npos, nrefine, stab_margin = args
    theta = cl_dyn.pack_theta(cand["a"], cand["n"])
    try:
        alpha, _ = cl_dyn.spectral_abscissa_and_grad(theta, eps_tol=1e-8, max_it=400, ncv=40)
    except Exception as e:
        return {**cand, "alpha": np.inf, "f_raw": np.inf, "omega_star": np.nan, "status": f"alpha_exception:{repr(e)}"}
    if alpha > -float(stab_margin):
        return {**cand, "alpha": float(alpha), "f_raw": np.inf, "omega_star": np.nan, "status": "unstable_or_margin"}
    try:
        om, gam, _info = find_peak_candidate_dynamic(
            cl_dyn, theta, wmin=float(wmin), wmax=float(wmax),
            npos=int(npos), nrefine=int(nrefine), xatol=1e-7, maxiter=60,
        )
        return {**cand, "alpha": float(alpha), "f_raw": float(gam), "omega_star": float(om), "status": "ok"}
    except Exception as e:
        return {**cand, "alpha": float(alpha), "f_raw": np.inf, "omega_star": np.nan, "status": f"hinf_exception:{repr(e)}"}


def _fdinit_write_candidate_summary(rows, filename):
    if not IS_WORLD_ROOT:
        return
    with open(filename, "w", newline="") as f:
        wr = csv.writer(f)
        wr.writerow(["candidate_id", "kind", "center", "amp", "width", "baseline", "fit_rel", "alpha", "f_raw", "omega_star", "norm_a", "norm_n", "status"])
        for r0 in rows:
            wr.writerow([
                r0["candidate_id"], r0["kind"], r0["center"], r0["amp"], r0["width"], r0["baseline"], r0["fit_rel"],
                r0.get("alpha", np.nan), r0.get("f_raw", np.inf), r0.get("omega_star", np.nan),
                float(np.linalg.norm(r0["a"])), float(np.linalg.norm(r0["n"])), r0.get("status", ""),
            ])


def run_frequency_domain_rational_init_pipeline(
    A, M, V, phi, bc, is_free, sigma_gauss,
    r=4, xa_fixed=-0.98, xs_fixed=0.95,
):
    r = int(r)
    xa_fixed = float(xa_fixed); xs_fixed = float(xs_fixed)
    _rprint("\n" + "#"*100)
    _rprint("FREQUENCY-DOMAIN RATIONAL INITIALIZATION, NO LQG, K=0 DIRECT FEEDTHROUGH")
    _rprint(f"fixed placement: xa={xa_fixed:+.10f}, xs={xs_fixed:+.10f}, r={r}")
    _rprint("pipeline: desired loop Ld -> target Kd=Ld/P -> rational fit -> true Hinf screening -> PyGRANSO")
    _rprint("#"*100 + "\n")

    b_vec, _ = build_actuator_vector(xa_fixed, sigma_gauss, V, phi, bc, is_free)
    s_vec, _ = build_sensor_state_vector(xs_fixed, sigma_gauss, V, is_free)
    c_vec = sensor_output_vector(M, s_vec)
    cl_dyn = ClosedLoopDynamicCompanionSISOStrictlyProper(A, M, b_vec, c_vec, r=r)

    ws = _fdinit_build_fit_grid()
    P = _fdinit_compute_siso_frequency_response(A, M, b_vec, c_vec, ws)
    cand = _fdinit_make_candidate_bank(ws, P, r=r)
    max_cand = int(os.environ.get("FDINIT_MAX_CANDIDATES", "0"))
    if max_cand > 0:
        cand = cand[:max_cand]
    _rprint(f"[fd-init] built {len(cand)} rational candidate(s)")

    # Save raw fits before expensive true-Hinf screening.
    if IS_WORLD_ROOT:
        with open("fdinit_rational_fit_bank.txt", "w") as f:
            for c0 in cand:
                f.write(f"candidate {c0['candidate_id']} kind={c0['kind']} center={c0['center']} amp={c0['amp']} width={c0['width']} baseline={c0['baseline']} fit_rel={c0['fit_rel']:.6e}\n")
                f.write("a = " + np.array2string(c0["a"], precision=12) + "\n")
                f.write("n = " + np.array2string(c0["n"], precision=12) + "\n\n")

    screen_top_by_fit = int(os.environ.get("FDINIT_SCREEN_TOP_BY_FIT", "40"))
    cand_screen = sorted(cand, key=lambda z: z["fit_rel"])[:min(screen_top_by_fit, len(cand))]
    _rprint(f"[fd-init] screening {len(cand_screen)} candidate(s) with true q->q Hinf")

    eval_items = [(
        cl_dyn, c0,
        float(os.environ.get("FDINIT_SCREEN_WMIN", "1e-4")),
        float(os.environ.get("FDINIT_SCREEN_WMAX", "5e1")),
        int(os.environ.get("FDINIT_SCREEN_NPOS", "35")),
        int(os.environ.get("FDINIT_SCREEN_NREFINE", "2")),
        float(os.environ.get("FDINIT_STAB_MARGIN", "1e-3")),
    ) for c0 in cand_screen]
    rows = mpi_task_map(_fdinit_evaluate_candidate, eval_items, label="fdinit_true_hinf_screen")
    rows = sorted(rows, key=lambda z: z.get("f_raw", np.inf))
    _fdinit_write_candidate_summary(rows, "fdinit_stage1_screen_summary.csv")

    feasible = [r0 for r0 in rows if np.isfinite(r0.get("f_raw", np.inf))]
    if not feasible:
        _rprint("[fd-init] no feasible screened candidate. Stopping before PyGRANSO.")
        return {"screen_rows": rows, "best": None}

    best0 = feasible[0]
    _rprint("\n[fd-init] best screened rational candidate:")
    _rprint(f"  id={best0['candidate_id']} kind={best0['kind']} center={best0['center']} amp={best0['amp']} width={best0['width']} baseline={best0['baseline']}")
    _rprint(f"  f={best0['f_raw']:.10e}, alpha={best0['alpha']:+.10e}, omega={best0['omega_star']:+.6e}, fit_rel={best0['fit_rel']:.3e}")
    _rprint(f"  a={best0['a']}")
    _rprint(f"  n={best0['n']}")

    joint_top = int(os.environ.get("FDINIT_JOINT_TOP", "5"))
    maxit = int(os.environ.get("FDINIT_JOINT_MAXIT", "60"))
    print_level = int(os.environ.get("FDINIT_PRINT_LEVEL", "0"))
    joint_rows = []
    best_res = None
    for j, c0 in enumerate(feasible[:min(joint_top, len(feasible))]):
        _rprint("\n" + "="*96)
        _rprint(f"[fd-init] joint PyGRANSO refinement {j+1}/{min(joint_top, len(feasible))}, candidate id={c0['candidate_id']}")
        _rprint("="*96)
        res = run_hinf_r4_noK_openloop_pygranso(
            A=A, M=M, V=V, phi=phi, bc=bc, is_free=is_free, sigma_gauss=sigma_gauss,
            r=r, xa_fixed=xa_fixed, xs_fixed=xs_fixed,
            a_center=c0["a"], n_center=c0["n"],
            a_scale=float(os.environ.get("FDINIT_A_SCALE", "1.0")),
            n_scale=float(os.environ.get("FDINIT_N_SCALE", "1.0")),
            K_fixed=0.0,
            maxit=maxit,
            print_level=print_level,
            verbose=_env_flag("FDINIT_VERBOSE", True),
            stab_margin=float(os.environ.get("HINF_STAB_MARGIN", os.environ.get("FDINIT_STAB_MARGIN", "1e-3"))),
            ...
            num_guess=int(os.environ.get("HINF_NUM_GUESS", "1")),
            wmin=float(os.environ.get("HINF_WMIN", "1e-4")),
            wmax=float(os.environ.get("HINF_WMAX", "5e1")),
            candidate_npos=int(os.environ.get("HINF_CANDIDATE_NPOS", "50")),
            ...
            base_seed=int(os.environ.get("FDINIT_BASE_SEED", "2025")) + 1009*j,
        )
        bbest = res.get("best", None)
        row = {
            "rank": j,
            "candidate_id": c0["candidate_id"],
            "init_f": c0["f_raw"],
            "init_alpha": c0["alpha"],
            "init_omega": c0["omega_star"],
            "best_f": np.inf if bbest is None else bbest["f_raw"],
            "best_alpha": np.nan if bbest is None else bbest["alpha"],
            "best_omega": np.nan if bbest is None else bbest["omega_star"],
            "best_a": None if bbest is None else bbest["a"],
            "best_n": None if bbest is None else bbest["n"],
        }
        joint_rows.append(row)
        if bbest is not None and (best_res is None or bbest["f_raw"] < best_res["best_f"]):
            best_res = row

    if IS_WORLD_ROOT:
        with open("fdinit_stage2_joint_summary.csv", "w", newline="") as f:
            wr = csv.writer(f)
            wr.writerow(["rank", "candidate_id", "init_f", "init_alpha", "init_omega", "best_f", "best_alpha", "best_omega"])
            for r0 in joint_rows:
                wr.writerow([r0["rank"], r0["candidate_id"], r0["init_f"], r0["init_alpha"], r0["init_omega"], r0["best_f"], r0["best_alpha"], r0["best_omega"]])
        if best_res is not None:
            with open("fdinit_global_best.txt", "w") as f:
                f.write(f"best_f = {best_res['best_f']:.12e}\n")
                f.write(f"best_alpha = {best_res['best_alpha']:.12e}\n")
                f.write(f"best_omega = {best_res['best_omega']:.12e}\n")
                f.write(f"source_candidate_id = {best_res['candidate_id']}\n")
                f.write("best_a = " + np.array2string(best_res["best_a"], precision=12) + "\n")
                f.write("best_n = " + np.array2string(best_res["best_n"], precision=12) + "\n")

    _rprint("\n" + "#"*100)
    _rprint("FREQUENCY-DOMAIN INITIALIZATION DONE")
    if best_res is None:
        _rprint("No feasible refined result.")
    else:
        _rprint(f"best refined f     = {best_res['best_f']:.10e}")
        _rprint(f"best refined alpha = {best_res['best_alpha']:+.10e}")
        _rprint(f"best refined omega = {best_res['best_omega']:+.6e}")
        _rprint(f"best a             = {best_res['best_a']}")
        _rprint(f"best n             = {best_res['best_n']}")
    _rprint("#"*100 + "\n")

    return {"screen_rows": rows, "joint_rows": joint_rows, "best": best_res}


# If requested, run this pipeline and exit before the default multistart block.


# -----------------------------------------------------------------------------
# FREQUENCY-DOMAIN INITIALIZATION v2: rational fit + numerator scale/phase screen
# -----------------------------------------------------------------------------
# The first FDINIT attempt often produced rational fits whose full numerator was
# immediately destabilizing.  This v2 treats the rational fit as a *direction*
# and explicitly searches over controller gain/phase before Hinf refinement.

def _fdinit_scaled_parse_complex_phases(name="FDINIT_PHASES", default="0,1.57079632679,3.14159265359,4.71238898038"):
    vals = _fdinit_parse_float_list(name, default)
    return [np.exp(1j*float(phi)) for phi in vals]


def _fdinit_desired_loop_scaled(ws, amp=1.0, width=0.45, center=-0.54, baseline=0.0, kind="gaussian", sign=-1.0):
    """Desired loop for positive-feedback convention S=1/(1-L).

    sign=-1 gives the conservative negative-real target Ld=-A(w).
    sign=+1 tests the opposite sign, useful for catching convention/phase issues.
    """
    base = _fdinit_desired_loop(ws, amp=amp, width=width, center=center, baseline=baseline, kind=kind)
    # _fdinit_desired_loop returns -A. sign=-1 should return -A, sign=+1 returns +A.
    return base if float(sign) < 0 else -base


def _fdinit_make_candidate_bank_scaled(ws, P, r=4):
    centers = _fdinit_parse_float_list("FDINIT_LD_CENTERS", "-0.65,-0.54,-0.42,0.0")
    amps = _fdinit_parse_float_list("FDINIT_LD_AMPS", "0.02,0.05,0.1,0.2,0.5,1,2,5,10")
    widths = _fdinit_parse_float_list("FDINIT_LD_WIDTHS", "0.18,0.30,0.45,0.70")
    baselines = _fdinit_parse_float_list("FDINIT_LD_BASELINES", "0,0.1,1")
    signs = _fdinit_parse_float_list("FDINIT_LD_SIGNS", "-1,1")
    kinds = [x.strip() for x in os.environ.get("FDINIT_LD_KINDS", "gaussian,lorentz,two_peak").split(',') if x.strip()]
    epsP = float(os.environ.get("FDINIT_MIN_ABSP", "1e-10"))
        ...
        for sign in signs:
            for center in centers:
                for amp in amps:
                    for width in widths:
                        for baseline in baselines:
                            Ld = _fdinit_desired_loop_scaled(ws, amp=amp, width=width, center=center, baseline=baseline, kind=kind, sign=sign)
                            P_safe = np.where(np.abs(P) > epsP, P, epsP * np.exp(1j*np.angle(P + 1e-30)))
                            Kd = Ld / P_safe
                            # Emphasize frequencies where action is desired, but avoid forcing huge fits where P is tiny.
                            fit_weight = (np.abs(Ld) + 0.05) * np.minimum(1.0, np.abs(P) / (np.median(np.abs(P)) + 1e-30))
                            a, b = _fdinit_rational_fit_strictly_proper(
                                ws, Kd, r=r, weights=fit_weight,
                                sk_iters=int(os.environ.get("FDINIT_SK_ITERS", "4")),
                                stable_reflect=True,
                            ...
                            fit_rel = float(np.linalg.norm(np.sqrt(fit_weight)*(Kfit-Kd)) / (np.linalg.norm(np.sqrt(fit_weight)*Kd) + 1e-30))
                            nvec = np.conjugate(b)
                            # Score used only to choose which candidates to screen.  Penalize huge raw numerator directions.
                            score = fit_rel + float(os.environ.get("FDINIT_NORM_SCORE_PENALTY", "0.03")) * np.log10(1.0 + float(np.linalg.norm(nvec)))
                            cand.append({
                                "candidate_id": cid,
                                "kind": kind,
                                "sign": float(sign),
...
def _fdinit_scaled_evaluate_candidate(args):
    cl_dyn, cand, wmin, wmax, npos, nrefine, stab_margin, scales, phases = args
    best = None
    base_n = np.asarray(cand["n"], dtype=np.complex128)
    for phase in phases:
        for rho in scales:
            ctest = dict(cand)
            ctest["scale"] = float(rho)
            ctest["phase_angle"] = float(np.angle(phase))
            ctest["n"] = (complex(phase) * float(rho) * base_n).astype(np.complex128)
            theta = cl_dyn.pack_theta(ctest["a"], ctest["n"])
            try:
                alpha, _ = cl_dyn.spectral_abscissa_and_grad(theta, eps_tol=1e-8, max_it=400, ncv=40)
            except Exception as e:
                row = {**ctest, "alpha": np.inf, "f_raw": np.inf, "omega_star": np.nan, "status": f"alpha_exception:{repr(e)}"}
                if best is None:
                    best = row
                continue
            if alpha > -float(stab_margin):
                row = {**ctest, "alpha": float(alpha), "f_raw": np.inf, "omega_star": np.nan, "status": "unstable_or_margin"}
            else:
                try:
                    om, gam, _info = find_peak_candidate_dynamic(
                        cl_dyn, theta, wmin=float(wmin), wmax=float(wmax),
                    ...
                    row = {**ctest, "alpha": float(alpha), "f_raw": float(gam), "omega_star": float(om), "status": "ok"}
                except Exception as e:
                    row = {**ctest, "alpha": float(alpha), "f_raw": np.inf, "omega_star": np.nan, "status": f"hinf_exception:{repr(e)}"}
            if best is None or row.get("f_raw", np.inf) < best.get("f_raw", np.inf):
                best = row
    return best


def _fdinit_write_scaled_candidate_summary(rows, filename):
    if not IS_WORLD_ROOT:
        return
    with open(filename, "w", newline="") as f:
        wr = csv.writer(f)
        wr.writerow(["candidate_id", "kind", "sign", "center", "amp", "width", "baseline", "fit_rel", "screen_score", "scale", "phase_angle", "alpha", "f_raw", "omega_star", "norm_a", "norm_n", "status"])
        for r0 in rows:
            wr.writerow([
                r0.get("candidate_id"), r0.get("kind"), r0.get("sign", np.nan), r0.get("center"), r0.get("amp"), r0.get("width"), r0.get("baseline"),
                r0.get("fit_rel", np.nan), r0.get("screen_score", np.nan), r0.get("scale", np.nan), r0.get("phase_angle", np.nan),
                r0.get("alpha", np.nan), r0.get("f_raw", np.inf), r0.get("omega_star", np.nan),
                float(np.linalg.norm(r0["a"])), float(np.linalg.norm(r0["n"])), r0.get("status", ""),
            ])


def run_frequency_domain_scaled_init_pipeline(
    A, M, V, phi, bc, is_free, sigma_gauss,
    r=4, xa_fixed=-0.98, xs_fixed=0.95,
):
    r = int(r)
    xa_fixed = float(xa_fixed); xs_fixed = float(xs_fixed)
    _rprint("\n" + "#"*100)
    _rprint("FREQUENCY-DOMAIN RATIONAL INITIALIZATION v2: FIT DIRECTION + SCALE/PHASE SCREEN, K=0")
    _rprint(f"fixed placement: xa={xa_fixed:+.10f}, xs={xs_fixed:+.10f}, r={r}")
    _rprint("pipeline: desired loop -> Kd=Ld/P -> rational fit -> scale/phase screen -> joint PyGRANSO")
    _rprint("#"*100 + "\n")
    ...
    ws = _fdinit_build_fit_grid()
    P = _fdinit_compute_siso_frequency_response(A, M, b_vec, c_vec, ws)
    cand = _fdinit_make_candidate_bank_scaled(ws, P, r=r)
    max_cand = int(os.environ.get("FDINIT_MAX_CANDIDATES", "0"))
    if max_cand > 0:
        # score-sort before truncation, not generation-order truncation
        cand = sorted(cand, key=lambda z: z.get("screen_score", z.get("fit_rel", np.inf)))[:max_cand]
    _rprint(f"[fd-init-v2] built {len(cand)} rational candidate(s)")

    if IS_WORLD_ROOT:
        with open("fdinit_scaled_rational_fit_bank.txt", "w") as f:
            for c0 in cand:
                f.write(f"candidate {c0['candidate_id']} kind={c0['kind']} sign={c0['sign']} center={c0['center']} amp={c0['amp']} width={c0['width']} baseline={c0['baseline']} fit_rel={c0['fit_rel']:.6e} score={c0['screen_score']:.6e}\n")
                f.write("a = " + np.array2string(c0["a"], precision=12) + "\n")
                f.write("n_dir = " + np.array2string(c0["n"], precision=12) + "\n\n")

    screen_top = int(os.environ.get("FDINIT_SCREEN_TOP_BY_FIT", "80"))
    cand_screen = sorted(cand, key=lambda z: z.get("screen_score", z.get("fit_rel", np.inf)))[:min(screen_top, len(cand))]
    scales = np.asarray(_fdinit_parse_float_list("FDINIT_SCALE_LIST", "1e-4,3e-4,1e-3,3e-3,1e-2,3e-2,1e-1,3e-1,1"), dtype=float)
    phases = _fdinit_scaled_parse_complex_phases()
    _rprint(f"[fd-init-v2] screening {len(cand_screen)} candidate direction(s), {len(scales)} scale(s), {len(phases)} phase(s)")
    ...
    rows = mpi_task_map(_fdinit_scaled_evaluate_candidate, eval_items, label="fdinit_v2_scale_phase_screen")
    rows = sorted(rows, key=lambda z: z.get("f_raw", np.inf))
    _fdinit_write_scaled_candidate_summary(rows, "fdinit_v2_stage1_scale_phase_summary.csv")

    feasible = [r0 for r0 in rows if np.isfinite(r0.get("f_raw", np.inf))]
    if not feasible:
        _rprint("[fd-init-v2] no feasible screened candidate. Stopping before PyGRANSO.")
        return {"screen_rows": rows, "best": None}

    best0 = feasible[0]
    _rprint("\n[fd-init-v2] best screened rational direction after scale/phase:")
    _rprint(f"  id={best0['candidate_id']} kind={best0['kind']} sign={best0.get('sign')} center={best0['center']} amp={best0['amp']} width={best0['width']} baseline={best0['baseline']}")
    _rprint(f"  scale={best0.get('scale')}, phase={best0.get('phase_angle')}, f={best0['f_raw']:.10e}, alpha={best0['alpha']:+.10e}, omega={best0['omega_star']:+.6e}")
    _rprint(f"  a={best0['a']}")
    _rprint(f"  n={best0['n']}")
    ...
    for j, c0 in enumerate(feasible[:min(joint_top, len(feasible))]):
        _rprint("\n" + "="*96)
        _rprint(f"[fd-init-v2] joint PyGRANSO refinement {j+1}/{min(joint_top, len(feasible))}, candidate id={c0['candidate_id']}")
        _rprint("="*96)
        res = run_hinf_r4_noK_openloop_pygranso(
            A=A, M=M, V=V, phi=phi, bc=bc, is_free=is_free, sigma_gauss=sigma_gauss,
            r=r, xa_fixed=xa_fixed, xs_fixed=xs_fixed,
        ...
        joint_rows.append({
            ...
            "candidate_id": c0["candidate_id"],
            "kind": c0["kind"],
            "sign": c0.get("sign", np.nan),
            "center": c0["center"],
            "amp": c0["amp"],
            "width": c0["width"],
            "baseline": c0["baseline"],
            "init_scale": c0.get("scale", np.nan),
            "init_phase": c0.get("phase_angle", np.nan),
            "init_f": c0.get("f_raw", np.inf),
            "init_alpha": c0.get("alpha", np.nan),
            "best_f": res.get("best_f", np.inf),
            "best_alpha": res.get("best_alpha", np.nan),
            "best_omega": res.get("best_omega", np.nan),
            "norm_n": float(np.linalg.norm(res.get("best_n", np.zeros(r)))),
            "status": "ok" if np.isfinite(res.get("best_f", np.inf)) else "fail",
        })
        if best_res is None or res.get("best_f", np.inf) < best_res.get("best_f", np.inf):
            best_res = res

    if IS_WORLD_ROOT:
        with open("fdinit_v2_stage2_joint_summary.csv", "w", newline="") as f:
            wr = csv.DictWriter(f, fieldnames=list(joint_rows[0].keys()) if joint_rows else [])
            if joint_rows:
                wr.writeheader(); wr.writerows(joint_rows)
        if best_res is not None:
            with open("fdinit_v2_global_best.txt", "w") as f:
                f.write("FREQUENCY-DOMAIN INIT v2 GLOBAL BEST\n")
                f.write(f"best_f = {best_res.get('best_f', np.inf):.16e}\n")
                f.write(f"best_alpha = {best_res.get('best_alpha', np.nan):.16e}\n")
                f.write(f"best_omega = {best_res.get('best_omega', np.nan):.16e}\n")
                f.write("best_a = " + np.array2string(best_res.get("best_a", np.zeros(r)), precision=16) + "\n")
                f.write("best_n = " + np.array2string(best_res.get("best_n", np.zeros(r)), precision=16) + "\n")
    return {"screen_rows": rows, "joint_rows": joint_rows, "best": best_res}


# =============================================================================
# FDINIT v2: DIRECT PyGRANSO FROM ONE SCREENED CANDIDATE
# =============================================================================
# This bypasses the scale/phase screen and launches PyGRANSO directly from one
# screened candidate.  By default it reads fdinit_v2_stage1_scale_phase_summary.csv
# and picks the finite row with smallest f_raw.  The candidate bank is regenerated
# deterministically, then the selected scale and phase are applied to its numerator.
#
# Environment knobs:
#   RUN_FDINIT_DIRECT_CANDIDATE=1
#   FDINIT_DIRECT_CSV=fdinit_v2_stage1_scale_phase_summary.csv
#   FDINIT_DIRECT_CANDIDATE_ID=<int>     # optional; otherwise best finite csv row
#   FDINIT_DIRECT_SCALE=<float>          # optional override
#   FDINIT_DIRECT_PHASE=<float radians>  # optional override
#   FDINIT_DIRECT_MAXIT=60
#   FDINIT_DIRECT_A_SCALE=1.0
#   FDINIT_DIRECT_N_SCALE=1.0
# =============================================================================

def _fdinit_direct_read_best_csv_row(csv_path, forced_candidate_id=None):
    if not IS_WORLD_ROOT:
        return None
    rows = []
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"Could not find FDINIT_DIRECT_CSV={csv_path!r}")
    with open(csv_path, "r", newline="") as f:
        rd = csv.DictReader(f)
        for row in rd:
            try:
                row["candidate_id"] = int(float(row.get("candidate_id", "nan")))
                row["scale"] = float(row.get("scale", "nan"))
                row["phase_angle"] = float(row.get("phase_angle", "nan"))
                row["f_raw"] = float(row.get("f_raw", "inf"))
                row["alpha"] = float(row.get("alpha", "nan"))
                row["omega_star"] = float(row.get("omega_star", "nan"))
            except Exception:
                continue
            if forced_candidate_id is not None and row["candidate_id"] != int(forced_candidate_id):
                continue
            if np.isfinite(row.get("f_raw", np.inf)) and str(row.get("status", "")).lower().startswith("ok"):
                rows.append(row)
    if len(rows) == 0:
        raise RuntimeError("No finite ok rows found in FDINIT_DIRECT_CSV for the requested candidate.")
    return min(rows, key=lambda z: z["f_raw"])


def _fdinit_direct_bcast_row(row):
    return WORLD.bcast(row if IS_WORLD_ROOT else None, root=0)


def run_fdinit_v2_direct_candidate_pygranso(
    A, M, V, phi, bc, is_free, sigma_gauss,
    r=4, xa_fixed=-0.98, xs_fixed=0.95,
):
    r = int(r)
    xa_fixed = float(xa_fixed); xs_fixed = float(xs_fixed)
    forced_id_env = os.environ.get("FDINIT_DIRECT_CANDIDATE_ID", "").strip()
    forced_id = int(forced_id_env) if forced_id_env else None
    csv_path = os.environ.get("FDINIT_DIRECT_CSV", "fdinit_v2_stage1_scale_phase_summary.csv")

    selected = _fdinit_direct_read_best_csv_row(csv_path, forced_candidate_id=forced_id)
    selected = _fdinit_direct_bcast_row(selected)
    cid = int(selected["candidate_id"])
    scale = float(os.environ.get("FDINIT_DIRECT_SCALE", selected["scale"]))
    phase_angle = float(os.environ.get("FDINIT_DIRECT_PHASE", selected["phase_angle"]))

    _rprint("\n" + "#"*100)
    _rprint("FDINIT v2 DIRECT CANDIDATE -> PyGRANSO, K=0")
    _rprint(f"fixed placement: xa={xa_fixed:+.10f}, xs={xs_fixed:+.10f}, r={r}")
    _rprint(f"csv row: candidate_id={cid}, csv_f={selected.get('f_raw'):.10e}, csv_alpha={selected.get('alpha'):+.10e}, csv_omega={selected.get('omega_star'):+.6e}")
    _rprint(f"using scale={scale:.16e}, phase_angle={phase_angle:.16e} rad")
    _rprint("#"*100 + "\n")

    # Regenerate the same deterministic candidate bank, then pick the requested id.
    b_vec, _ = build_actuator_vector(xa_fixed, sigma_gauss, V, phi, bc, is_free)
    s_vec, _ = build_sensor_state_vector(xs_fixed, sigma_gauss, V, is_free)
    c_vec = sensor_output_vector(M, s_vec)
    cl_dyn = ClosedLoopDynamicCompanionSISOStrictlyProper(A, M, b_vec, c_vec, r=r)
    ...
    P = _fdinit_compute_siso_frequency_response(A, M, b_vec, c_vec, ws)
    cand = _fdinit_make_candidate_bank_scaled(ws, P, r=r)
    matches = [c0 for c0 in cand if int(c0["candidate_id"]) == cid]
    if len(matches) == 0:
        raise RuntimeError(
            f"Candidate id {cid} was not found in regenerated bank. "
            "Use the same FDINIT_LD_AMPS / width / baseline environment as in the screening run, "
            "or set FDINIT_DIRECT_CANDIDATE_ID to an id present in the current bank."
        )
    c0 = matches[0]
    a0 = np.asarray(c0["a"], dtype=np.complex128).reshape(r)
    n0 = (np.exp(1j * phase_angle) * scale * np.asarray(c0["n"], dtype=np.complex128).reshape(r)).astype(np.complex128)

    # Sanity-check the starting candidate with the same true objective.
    theta0 = cl_dyn.pack_theta(a0, n0)
    try:
        alpha0, _ = cl_dyn.spectral_abscissa_and_grad(theta0, eps_tol=1e-8, max_it=400, ncv=40)
        if alpha0 <= -float(os.environ.get("FDINIT_STAB_MARGIN", "1e-3")):
            om0, gam0, _ = find_peak_candidate_dynamic(
                cl_dyn, theta0,
                wmin=float(os.environ.get("FDINIT_SCREEN_WMIN", "1e-4")),
                wmax=float(os.environ.get("FDINIT_SCREEN_WMAX", "5e1")),
                npos=int(os.environ.get("FDINIT_SCREEN_NPOS", "35")),
                nrefine=int(os.environ.get("FDINIT_SCREEN_NREFINE", "2")),
                xatol=1e-7, maxiter=60,
            )
        else:
            om0, gam0 = np.nan, np.inf
    except Exception as e:
        _rprint(f"[fd-init-v2-direct] initial sanity check failed: {repr(e)}")
        alpha0, om0, gam0 = np.inf, np.nan, np.inf

    _rprint("[fd-init-v2-direct] exact initial point regenerated from candidate bank:")
    _rprint(f"  candidate_id={cid} kind={c0.get('kind')} sign={c0.get('sign')} center={c0.get('center')} amp={c0.get('amp')} width={c0.get('width')} baseline={c0.get('baseline')}")
    _rprint(f"  initial Hinf={gam0:.10e}, alpha={alpha0:+.10e}, omega={om0:+.6e}")
    _rprint(f"  ||a0||={np.linalg.norm(a0):.6e}, ||n0||={np.linalg.norm(n0):.6e}")
    _rprint(f"  a0={a0}")
    _rprint(f"  n0={n0}")

    # Adaptive scale defaults: make z=O(1) correspond to a moderate relative perturbation.
    a_scale = float(os.environ.get("FDINIT_DIRECT_A_SCALE", os.environ.get("FDINIT_A_SCALE", max(1.0, 0.02*np.linalg.norm(a0)/np.sqrt(r)))))
    n_scale = float(os.environ.get("FDINIT_DIRECT_N_SCALE", os.environ.get("FDINIT_N_SCALE", max(1.0, 0.05*np.linalg.norm(n0)/np.sqrt(r)))))
    _rprint(f"[fd-init-v2-direct] PyGRANSO scales: a_scale={a_scale:.6e}, n_scale={n_scale:.6e}")

    res = run_hinf_r4_noK_openloop_pygranso(
        A=A, M=M, V=V, phi=phi, bc=bc, is_free=is_free, sigma_gauss=sigma_gauss,
        r=r, xa_fixed=xa_fixed, xs_fixed=xs_fixed,
        a_center=a0, n_center=n0,
        a_scale=a_scale,
        n_scale=n_scale,
        maxit=int(os.environ.get("FDINIT_DIRECT_MAXIT", os.environ.get("FDINIT_JOINT_MAXIT", "60"))),
        print_level=int(os.environ.get("FDINIT_DIRECT_PRINT_LEVEL", os.environ.get("FDINIT_PRINT_LEVEL", "0"))),
        verbose=_env_flag("FDINIT_DIRECT_VERBOSE", _env_flag("FDINIT_VERBOSE", True)),
        stab_margin=float(os.environ.get("HINF_STAB_MARGIN", os.environ.get("FDINIT_STAB_MARGIN", "1e-3"))),
        base_seed=int(os.environ.get("FDINIT_BASE_SEED", "2025")),
        # Use a more global/deterministic Hinf peak search during PyGRANSO.
        # The earlier direct script used the combined-function defaults
        # (notably num_guess=1), which can miss competing frequency peaks.
        num_guess=int(os.environ.get("FDINIT_DIRECT_NUM_GUESS", os.environ.get("HINF_NUM_GUESS", "8"))),
        candidate_npos=int(os.environ.get("FDINIT_DIRECT_CANDIDATE_NPOS", os.environ.get("HINF_CANDIDATE_NPOS", "160"))),
        wmin=float(os.environ.get("FDINIT_DIRECT_WMIN", os.environ.get("HINF_WMIN", "1e-4"))),
        wmax=float(os.environ.get("FDINIT_DIRECT_WMAX", os.environ.get("HINF_WMAX", "5e1"))),
        disk_b=float(os.environ.get("FDINIT_DIRECT_DISK_B", os.environ.get("HINF_DISK_B", "50.0"))),
        disk_nev=int(os.environ.get("FDINIT_DIRECT_DISK_NEV", os.environ.get("HINF_DISK_NEV", "80"))),
        disk_eigs_tol=float(os.environ.get("FDINIT_DIRECT_DISK_EIGS_TOL", os.environ.get("HINF_DISK_EIGS_TOL", "1e-8"))),
        disk_eps_probe=float(os.environ.get("FDINIT_DIRECT_DISK_EPS_PROBE", os.environ.get("HINF_DISK_EPS_PROBE", "1e-6"))),
        disk_axis_warn=float(os.environ.get("FDINIT_DIRECT_DISK_AXIS_WARN", os.environ.get("HINF_DISK_AXIS_WARN", "1e-3"))),
    )
    best = res.get("best")
    if IS_WORLD_ROOT:
        with open("fdinit_v2_direct_candidate_summary.csv", "w", newline="") as f:
            wr = csv.writer(f)
            wr.writerow(["candidate_id", "scale", "phase_angle", "init_f", "init_alpha", "init_omega", "best_f", "best_alpha", "best_omega", "norm_a", "norm_n", "status"])
            if best is None:
                wr.writerow([cid, scale, phase_angle, gam0, alpha0, om0, np.inf, np.nan, np.nan, np.linalg.norm(a0), np.linalg.norm(n0), "fail"])
            else:
                wr.writerow([cid, scale, phase_angle, gam0, alpha0, om0, best["f_raw"], best["alpha"], best["omega_star"], np.linalg.norm(best["a"]), np.linalg.norm(best["n"]), "ok"])
        if best is not None:
            with open("fdinit_v2_direct_candidate_best.txt", "w") as f:
                f.write("FDINIT v2 DIRECT CANDIDATE BEST\n")
                f.write(f"candidate_id = {cid}\n")
                f.write(f"init_f = {gam0:.16e}\n")
                f.write(f"init_alpha = {alpha0:.16e}\n")
                f.write(f"init_omega = {om0:.16e}\n")
                f.write(f"best_f = {best['f_raw']:.16e}\n")
                f.write(f"best_alpha = {best['alpha']:.16e}\n")
                f.write(f"best_omega = {best['omega_star']:.16e}\n")
                f.write("best_a = " + np.array2string(best["a"], precision=16) + "\n")
                f.write("best_n = " + np.array2string(best["n"], precision=16) + "\n")
    # Optional final high-resolution rescore of the selected best iterate.
    # This is important when the optimizer history shows multiple competing
    # peaks at nearly the same controller parameters.
    rescore = None
    if best is not None:
        try:
            theta_best = cl_dyn.pack_theta(best["a"], best["n"])
            rescore = dynamic_oracle_hard_stability(
                cl_dyn,
                theta=theta_best,
                stab_margin=float(os.environ.get("HINF_STAB_MARGIN", os.environ.get("FDINIT_STAB_MARGIN", "1e-3"))),
                stab_buffer=float(os.environ.get("FDINIT_DIRECT_RESCORE_STAB_BUFFER", "0.0")),
                num_guess=int(os.environ.get("FDINIT_DIRECT_RESCORE_NUM_GUESS", os.environ.get("FDINIT_DIRECT_NUM_GUESS", "16"))),
                candidate_npos=int(os.environ.get("FDINIT_DIRECT_RESCORE_CANDIDATE_NPOS", os.environ.get("FDINIT_DIRECT_CANDIDATE_NPOS", "300"))),
                wmin=float(os.environ.get("FDINIT_DIRECT_RESCORE_WMIN", os.environ.get("FDINIT_DIRECT_WMIN", "1e-5"))),
                wmax=float(os.environ.get("FDINIT_DIRECT_RESCORE_WMAX", os.environ.get("FDINIT_DIRECT_WMAX", "1e2"))),
                disk_b=float(os.environ.get("FDINIT_DIRECT_RESCORE_DISK_B", os.environ.get("FDINIT_DIRECT_DISK_B", "100.0"))),
                disk_nev=int(os.environ.get("FDINIT_DIRECT_RESCORE_DISK_NEV", os.environ.get("FDINIT_DIRECT_DISK_NEV", "120"))),
                disk_eigs_tol=float(os.environ.get("FDINIT_DIRECT_RESCORE_DISK_EIGS_TOL", os.environ.get("FDINIT_DIRECT_DISK_EIGS_TOL", "1e-9"))),
                disk_eps_probe=float(os.environ.get("FDINIT_DIRECT_RESCORE_DISK_EPS_PROBE", os.environ.get("FDINIT_DIRECT_DISK_EPS_PROBE", "1e-7"))),
                disk_axis_warn=float(os.environ.get("FDINIT_DIRECT_RESCORE_DISK_AXIS_WARN", os.environ.get("FDINIT_DIRECT_DISK_AXIS_WARN", "1e-4"))),
                base_seed=int(os.environ.get("FDINIT_BASE_SEED", "2025")),
            )
            if IS_WORLD_ROOT:
                _rprint("\n[fd-init-v2-direct] high-resolution rescore of best iterate:")
                _rprint(f"  rescore_f     = {rescore['f_raw']:.10e}")
                _rprint(f"  rescore_alpha = {rescore['alpha']:+.10e}")
                _rprint(f"  rescore_omega = {rescore['omega_star']:+.6e}")
                with open("fdinit_v2_direct_candidate_best_rescore.txt", "w") as f:
                    f.write("FDINIT v2 DIRECT CANDIDATE BEST RESCORE\n")
                    f.write(f"candidate_id = {cid}\n")
                    f.write(f"history_best_f = {best['f_raw']:.16e}\n")
                    f.write(f"history_best_alpha = {best['alpha']:.16e}\n")
                    f.write(f"history_best_omega = {best['omega_star']:.16e}\n")
                    f.write(f"rescore_f = {rescore['f_raw']:.16e}\n")
                    f.write(f"rescore_alpha = {rescore['alpha']:.16e}\n")
                    f.write(f"rescore_omega = {rescore['omega_star']:.16e}\n")
                    f.write(f"rescore_certified = {bool(rescore.get('certified', False))}\n")
        except Exception as e:
            if IS_WORLD_ROOT:
                _rprint(f"[fd-init-v2-direct] high-resolution rescore failed: {repr(e)}")

    return {"selected": selected, "best": best, "rescore": rescore, "raw_result": res}



if _env_flag("RUN_FDINIT_DIRECT_CANDIDATE", False):
    try:
        run_fdinit_v2_direct_candidate_pygranso(
            A=A, M=M, V=V, phi=phi, bc=bc, is_free=is_free, sigma_gauss=sigma_gauss,
            r=int(os.environ.get("FDINIT_R", "4")),
            xa_fixed=float(os.environ.get("FDINIT_XA", "-0.98")),
            xs_fixed=float(os.environ.get("FDINIT_XS", "0.95")),
        )
        if IS_WORLD_ROOT:
            print("RUN_FDINIT_DIRECT_CANDIDATE=1 complete; exiting before default multistart.", flush=True)
    finally:
        WORLD.Barrier()
    sys.exit(0)


if _env_flag("RUN_FDINIT_SCALED_PIPELINE", False):
    try:
        run_frequency_domain_scaled_init_pipeline(
            A=A, M=M, V=V, phi=phi, bc=bc, is_free=is_free, sigma_gauss=sigma_gauss,
            r=int(os.environ.get("FDINIT_R", "4")),
            xa_fixed=float(os.environ.get("FDINIT_XA", "-0.98")),
            xs_fixed=float(os.environ.get("FDINIT_XS", "0.95")),
        ...
        if IS_WORLD_ROOT:
            print("RUN_FDINIT_SCALED_PIPELINE=1 complete; exiting before default multistart.", flush=True)
    finally:
        WORLD.Barrier()
    sys.exit(0)


if _env_flag("RUN_FDINIT_PIPELINE", False):
    R4_FD_XA = float(os.environ.get("HINF_R4_NOK_XA", "-0.98"))
    R4_FD_XS = float(os.environ.get("HINF_R4_NOK_XS", "0.95"))
    fdinit_result = run_frequency_domain_rational_init_pipeline(
        A=A, M=M, V=V, phi=phi, bc=bc, is_free=is_free, sigma_gauss=sigma_gauss,
        r=int(os.environ.get("FDINIT_R", "4")),
        xa_fixed=R4_FD_XA,
        xs_fixed=R4_FD_XS,
    ...
    if IS_WORLD_ROOT:
        print("RUN_FDINIT_PIPELINE=1 complete; exiting before default multistart.", flush=True)
    raise SystemExit(0)



# -----------------------------------------------------------------------------
# Execute true controller multistart.
# -----------------------------------------------------------------------------
R4_MS_XA = float(os.environ.get("HINF_R4_NOK_XA", "-0.98"))
R4_MS_XS = float(os.environ.get("HINF_R4_NOK_XS", "0.95"))

hinf_r4_noK_multistart_run = run_hinf_r4_noK_true_multistart_openloop(
    A=A,
    M=M,
    V=V,
    phi=phi,
    ...
    xa_fixed=R4_MS_XA,
    xs_fixed=R4_MS_XS,
    nstarts=int(os.environ.get("HINF_MS_NSTARTS", "100")),
    master_seed=int(os.environ.get("HINF_MS_SEED", "123")),
    include_exact_openloop=_env_flag("HINF_MS_INCLUDE_EXACT_OPENLOOP", True),
    n_norm_min=float(os.environ.get("HINF_MS_N_NORM_MIN", "0.05")),
    n_norm_max=float(os.environ.get("HINF_MS_N_NORM_MAX", "5.0")),
    pole_jitter=float(os.environ.get("HINF_MS_POLE_JITTER", "0.35")),
    pole_imag_max=float(os.environ.get("HINF_MS_POLE_IMAG_MAX", "2.0")),
    a_scale=float(os.environ.get("HINF_R4_NOK_A_SCALE", "1.0")),
    n_scale=float(os.environ.get("HINF_R4_NOK_N_SCALE", "1.0")),
    K_fixed=0.0,
    maxit=int(os.environ.get("HINF_MAXIT", "40")),
    print_level=int(os.environ.get("HINF_MS_PRINT_LEVEL", "0")),
    verbose=_env_flag("HINF_MS_VERBOSE", True),

...
# -----------------------------------------------------------------------------
# Execute r=4 no-K Hinf run WARM-STARTED from balanced-truncated LQG companion file.
# This uses the same complex a/n parameterization as the open-loop experiment, but
# z0=0 now means exactly the loaded BT-LQG controller, not open loop.
# -----------------------------------------------------------------------------
R4_WS_FILE = os.environ.get("HINF_R4_WARMSTART_FILE", "lqg_r4_companion_warmstart.npz")
if not os.path.exists(R4_WS_FILE):
    raise FileNotFoundError(
        f"Warm-start file {R4_WS_FILE!r} not found. First run the serial export block "
        "lqg_r4_companion_warmstart_export_append.py to create it."
    )

_ws = np.load(R4_WS_FILE, allow_pickle=True)
R4_WS_R = int(np.asarray(_ws["r"]).item())
if R4_WS_R != 4:
    raise ValueError(f"Expected r=4 warm start, got r={R4_WS_R} in {R4_WS_FILE}")
R4_WS_XA = float(os.environ.get("HINF_R4_WS_XA", str(float(np.asarray(_ws["xa"]).item()))))
R4_WS_XS = float(os.environ.get("HINF_R4_WS_XS", str(float(np.asarray(_ws["xs"]).item()))))
R4_WS_A_CENTER = np.asarray(_ws["a_complex"], dtype=np.complex128).reshape(4)
R4_WS_N_CENTER = np.asarray(_ws["n_complex"], dtype=np.complex128).reshape(4)
R4_WS_K_COMPLEX = complex(np.asarray(_ws["K_complex"]).item())
if abs(R4_WS_K_COMPLEX) > 1e-8:
    _rprint(f"WARNING: warm-start file has nonzero feedthrough K={R4_WS_K_COMPLEX}; "
            "this no-K runner can only use real K_fixed. Proceeding with real(K).")
R4_WS_K_FIXED = float(np.real(R4_WS_K_COMPLEX))

_rprint("\n" + "#" * 100)
_rprint("HINF-ONLY r=4, NO-K, BT-LQG WARM START")
_rprint(f"warm-start file: {R4_WS_FILE}")
_rprint(f"fixed placement: xa={R4_WS_XA:+.10f}, xs={R4_WS_XS:+.10f}")
_rprint(f"loaded K feedthrough: {R4_WS_K_COMPLEX}")
_rprint("z=0 corresponds to loaded BT-LQG companion controller")
_rprint(f"a_center={R4_WS_A_CENTER}")
_rprint(f"n_center={R4_WS_N_CENTER}")
_rprint("#" * 100 + "\n")

hinf_r4_warm_run = run_hinf_r4_noK_openloop_pygranso(
    A=A,
    M=M,
    V=V,
    phi=phi,
    ...
    xa_fixed=R4_WS_XA,
    xs_fixed=R4_WS_XS,
    a_center=R4_WS_A_CENTER,
    n_center=R4_WS_N_CENTER,
    # These scales define how much physical change one unit of PyGRANSO z makes.
    # Since we are already at a good controller, start with small/medium scales.
    a_scale=float(os.environ.get("HINF_R4_WS_A_SCALE", "1.0")),
    n_scale=float(os.environ.get("HINF_R4_WS_N_SCALE", "1.0")),
    K_fixed=R4_WS_K_FIXED,
    maxit=int(os.environ.get("HINF_MAXIT", "40")),
    print_level=int(os.environ.get("HINF_PRINT_LEVEL", "1")),
    verbose=True,
    ...
    num_guess=int(os.environ.get("HINF_NUM_GUESS", "25")),
    ...
    base_seed=int(os.environ.get("HINF_BASE_SEED", "2")),
)

# Rewrite warm-start-specific CSV/TXT names so they don't get confused with open-loop runs.
if MPI.COMM_WORLD.rank == 0:
    import shutil as _shutil
    if os.path.exists("hinf_r4_noK_openloop_history.csv"):
        _shutil.copyfile("hinf_r4_noK_openloop_history.csv", "hinf_r4_noK_lqg_warmstart_history.csv")
    if os.path.exists("hinf_r4_noK_openloop_best.txt"):
        _shutil.copyfile("hinf_r4_noK_openloop_best.txt", "hinf_r4_noK_lqg_warmstart_best.txt")

# Diagnostic loop metrics at the best design. These are NOT optimization constraints.
if hinf_r4_warm_run["best"] is not None:
    b_best = hinf_r4_warm_run["best"]
    b_vec, _ = build_actuator_vector(R4_WS_XA, sigma_gauss, V, phi, bc, is_free)
    s_vec_best, _ = build_sensor_state_vector(R4_WS_XS, sigma_gauss, V, is_free)
    c_vec_best = sensor_output_vector(M, s_vec_best)
    fine_loop = _robust_loop_metrics_dynamic(
        A, M, b_vec, c_vec_best, b_best["a"], b_best["n"], R4_WS_K_FIXED,
        wmin=1e-5, wmax=5e1, npos=160,
        csv_name="hinf_r4_noK_lqg_warmstart_best_loop_fine.csv",
    )
    _rprint("Diagnostic loop check for best Hinf-only r=4 no-K BT-LQG-warmstarted design:")

...
# =============================================================================
# LANDSCAPE DIAGNOSTICS: 1D path scans + 2D basin slice, no-K, r=4
# =============================================================================
# Paste this block after the dynamic-controller helper functions/classes are
# defined and BEFORE any expensive PyGRANSO/multistart execution block.
# Then run with:
#   RUN_LANDSCAPE_DIAGNOSTICS=1 PYTHONUNBUFFERED=1 MPI_TASK_GROUP_SIZE=1 \
#   srun --mpi=pmi2 -n 40 python -u your_file.py
#
# Outputs:
#   landscape_path_<A>_to_<B>.csv/.png
#   landscape_slice_open_good_random.csv
#   landscape_slice_log10Hinf.png
#   landscape_slice_alpha.png
#   landscape_slice_omega.png
#   landscape_slice_stable_mask.png
# =============================================================================

import sys as _sys_landscape
import csv as _csv_landscape
import os as _os_landscape
import numpy as _np_landscape


def _landscape_safe_label(s):
    return str(s).replace(" ", "_").replace("/", "_").replace("-", "m").replace("+", "p")


def _landscape_default_a_r4():
    # Default stable denominator corresponding to the center used in your r=4 runs.
    # Falls back to the hard-coded center if default_stable_a_center is unavailable.
    try:
        return _np_landscape.asarray(default_stable_a_center(4), dtype=_np_landscape.complex128)
    except Exception:
        return _np_landscape.array([4.4, 16.3, 18.6, 7.7], dtype=_np_landscape.complex128)


def _landscape_pack(cl, a, n, K=0.0):
    """Pack complex a/n with K fixed to zero in the full dynamic class."""
    return cl.pack_theta(
        _np_landscape.asarray(a, dtype=_np_landscape.complex128),
        _np_landscape.asarray(n, dtype=_np_landscape.complex128),
        Kstat=float(K),
    )


def _landscape_try_load_npz_controller(path, label):
    """Load a controller from an npz with a_complex/n_complex keys."""
    if not path or not _os_landscape.path.exists(path):
        return None
    ws = _np_landscape.load(path, allow_pickle=True)
    if "a_complex" not in ws.files or "n_complex" not in ws.files:
        return None
    a = _np_landscape.asarray(ws["a_complex"], dtype=_np_landscape.complex128).reshape(-1)
    n = _np_landscape.asarray(ws["n_complex"], dtype=_np_landscape.complex128).reshape(-1)
    if a.size != 4 or n.size != 4:
        return None
    return {"label": label, "a": a, "n": n, "K": 0.0, "source": path}


def _landscape_builtin_controllers():
    """
    Controllers used for diagnostics.

    The built-in 'random_best_noK_249' is the no-K exact-open-loop multistart
    best you reported.  The BT-LQG controller is loaded from
    lqg_r4_companion_warmstart.npz when available.

    To add your Hinf-improved controller, save an npz with keys a_complex and
    n_complex and set HINF_GOOD_NPZ=/path/to/file.npz.
    """
    ctrls = []

    ctrls.append({
        "label": "open",
        "a": _landscape_default_a_r4(),
        "n": _np_landscape.zeros(4, dtype=_np_landscape.complex128),
        "K": 0.0,
        "source": "default stable a, zero n",
    })

    # Your no-K multistart exact-open-loop best: Hinf about 249.
    ctrls.append({
        "label": "random_best_noK_249",
        "a": _np_landscape.array([
            9.427298541721 - 1.529299834267j,
            11.842025068125 - 4.477160627503j,
            20.676735603833 - 0.285253358743j,
            5.673385523318 + 1.157055998202j,
        ], dtype=_np_landscape.complex128),
        "n": _np_landscape.array([
            2.732816575778 - 15.396537948013j,
            -5.148699352804 - 12.872859332345j,
            -7.77179903 + 0.88491181j,
            -0.84762045 + 4.80789713j,
        ], dtype=_np_landscape.complex128),
        "K": 0.0,
        "source": "reported no-K exact-open-loop multistart best",
    })

    # Optional BT-LQG r=4 warm start.  Search current working directory first.
    for path, lab in [
        (_os_landscape.environ.get("BT_LQG_R4_NPZ", "lqg_r4_companion_warmstart.npz"), "BT_LQG_r4"),
        (_os_landscape.environ.get("HINF_GOOD_NPZ", ""), "Hinf_good"),
    ]:
        c = _landscape_try_load_npz_controller(path, lab)
        if c is not None:
            ctrls.append(c)
...
def _landscape_build_cldyn(xa_fixed, xs_fixed, r=4):
    """Build the r=4 no-K dynamic closed-loop object at fixed placement."""
    b_red, _ = build_actuator_vector(float(xa_fixed), sigma_gauss, V, phi, bc, is_free)
    s_red, _ = build_sensor_state_vector(float(xs_fixed), sigma_gauss, V, is_free)
    c_red = sensor_output_vector(M, s_red)
    cl = ClosedLoopDynamicCompanionSISO(
        A=A, M=M, b=b_red, c=c_red,
        r=int(r), optimize_mc_gain=False, fixed_mc_gain=1.0,
    )
    return cl, b_red, c_red


def _landscape_eval_theta(cl, theta, label="", wmin=1e-4, wmax=5e1, npos=30, nrefine=3,
                          alpha_margin=1e-3, loop_metrics=False, b_vec=None, c_vec=None,
                          loop_npos=45):
    """
    Evaluate one controller: alpha, sampled/refined signed Hinf, active omega,
    and optionally loop metrics.
    """
    out = {
        "label": label,
        "alpha": _np_landscape.nan,
        "c_stab": _np_landscape.nan,
        "Hinf": _np_landscape.inf,
        "omega_star": _np_landscape.nan,
        "stable": False,
        "Ms": _np_landscape.nan,
        "Mt": _np_landscape.nan,
        "KS": _np_landscape.nan,
        "min_return": _np_landscape.nan,
        "eval_error": "",
    }
    try:
        alpha, _ = cl.spectral_abscissa_and_grad(theta)
        out["alpha"] = float(alpha)
        out["c_stab"] = float(alpha + alpha_margin)
        out["stable"] = bool(alpha + alpha_margin <= 0.0)
        if not out["stable"]:
            return out

        w_star, g_star, _info = find_peak_candidate_dynamic(
            cl, theta,
            wmin=float(wmin), wmax=float(wmax),
            npos=int(npos), nrefine=int(nrefine),
            xatol=float(_os_landscape.environ.get("LAND_XATOL", "1e-7")),
            maxiter=int(_os_landscape.environ.get("LAND_REFINE_MAXITER", "60")),
        )
        out["Hinf"] = float(g_star)
        out["omega_star"] = float(w_star)

        if loop_metrics and b_vec is not None and c_vec is not None:
            a, nvec, _mc, Kstat = cl.unpack_theta(theta)
            lm = _robust_loop_metrics_dynamic(
                A, M, b_vec, c_vec, a, nvec, Kstat,
                wmin=float(_os_landscape.environ.get("LAND_LOOP_WMIN", "1e-4")),
                wmax=float(_os_landscape.environ.get("LAND_LOOP_WMAX", "5e1")),
                npos=int(loop_npos),
                csv_name=None,
            )
            out["Ms"] = float(lm["Ms"])
            out["Mt"] = float(lm["Mt"])
            out["KS"] = float(lm["KS"])
            out["min_return"] = float(lm["min_return"])
    except Exception as exc:
        out["eval_error"] = repr(exc)
    return out


def _landscape_write_rows_csv(filename, rows, fieldnames):
    if IS_WORLD_ROOT:
        with open(filename, "w", newline="") as f:
            wr = _csv_landscape.DictWriter(f, fieldnames=fieldnames)
            wr.writeheader()
            for r in rows:
                wr.writerow({k: r.get(k, "") for k in fieldnames})


def _landscape_plot_path(csv_name, png_name, title):
    if not IS_WORLD_ROOT:
        return
    try:
        import pandas as _pd
        import matplotlib.pyplot as _plt
        df = _pd.read_csv(csv_name)
        fig, axs = _plt.subplots(4, 1, figsize=(8.0, 10.0), sharex=True)
        stable = df["stable"].astype(bool).to_numpy()
        H = df["Hinf"].to_numpy()
        H_plot = _np_landscape.where(stable, H, _np_landscape.nan)
        axs[0].semilogy(df["t"], H_plot, marker="o")
        axs[0].set_ylabel("Hinf")
        axs[0].grid(True, which="both", alpha=0.35)
        axs[1].plot(df["t"], df["alpha"], marker="o")
        axs[1].axhline(0.0, linestyle="--", linewidth=1)
        axs[1].set_ylabel("alpha")
        axs[1].grid(True, alpha=0.35)
        axs[2].plot(df["t"], df["omega_star"], marker="o")
        axs[2].set_ylabel("omega*")
        axs[2].grid(True, alpha=0.35)
        if "min_return" in df.columns and _np_landscape.isfinite(df["min_return"]).any():
            axs[3].plot(df["t"], df["min_return"], marker="o")
            axs[3].set_ylabel("min |1-L|")
        else:
            axs[3].plot(df["t"], stable.astype(float), marker="o")
            axs[3].set_ylabel("stable")
        axs[3].set_xlabel("path parameter t")
        axs[3].grid(True, alpha=0.35)
        fig.suptitle(title)
        fig.tight_layout()
        fig.savefig(png_name, dpi=180)
        _plt.close(fig)
    except Exception as exc:
        print(f"Could not plot {png_name}: {exc}", flush=True)


def _landscape_plot_slice(csv_name, prefix="landscape_slice"):
    if not IS_WORLD_ROOT:
        return
    try:
        import pandas as _pd
        import matplotlib.pyplot as _plt
        df = _pd.read_csv(csv_name)
        ss = _np_landscape.sort(df["s"].unique())
        tt = _np_landscape.sort(df["t"].unique())
        def grid(col):
            Z = _np_landscape.full((len(tt), len(ss)), _np_landscape.nan)
            for _, row in df.iterrows():
                i = int(_np_landscape.where(tt == row["t"])[0][0])
                j = int(_np_landscape.where(ss == row["s"])[0][0])
                Z[i, j] = row[col]
            return Z
        S, T = _np_landscape.meshgrid(ss, tt)
        stable = grid("stable")
        H = grid("Hinf")
        Hlog = _np_landscape.log10(_np_landscape.where(_np_landscape.isfinite(H), H, _np_landscape.nan))
        alpha = grid("alpha")
        omega = grid("omega_star")

        for Z, name, title, cmap in [
            (Hlog, "log10Hinf", "log10 Hinf; open=(0,0), good=(1,0), random=(0,1)", None),
            (alpha, "alpha", "spectral abscissa alpha", None),
            (omega, "omega", "active peak frequency omega*", None),
            (stable, "stable_mask", "stable mask", None),
        ]:
            fig, ax = _plt.subplots(figsize=(7.2, 5.8))
            pcm = ax.pcolormesh(S, T, Z, shading="auto", cmap=cmap)
            ax.scatter([0, 1, 0], [0, 0, 1], marker="x", s=70)
            ax.text(0, 0, " open", va="bottom")
            ax.text(1, 0, " good", va="bottom")
            ax.text(0, 1, " random", va="bottom")
            ax.set_xlabel("s: open -> good direction")
            ax.set_ylabel("t: open -> random direction")
            ax.set_title(title)
            fig.colorbar(pcm, ax=ax)
            fig.tight_layout()
            fig.savefig(f"{prefix}_{name}.png", dpi=180)
            _plt.close(fig)
    except Exception as exc:
        print(f"Could not plot slice: {exc}", flush=True)


def run_landscape_path_and_slice_diagnostics():
    xa_fixed = float(_os_landscape.environ.get("LAND_XA", _os_landscape.environ.get("HINF_R4_NOK_XA", "-0.98")))
    xs_fixed = float(_os_landscape.environ.get("LAND_XS", _os_landscape.environ.get("HINF_R4_NOK_XS", "0.95")))
    n_path = int(_os_landscape.environ.get("LAND_PATH_N", "21"))
    n_slice = int(_os_landscape.environ.get("LAND_SLICE_N", "11"))
    s_min = float(_os_landscape.environ.get("LAND_SLICE_S_MIN", "-0.25"))
    s_max = float(_os_landscape.environ.get("LAND_SLICE_S_MAX", "1.25"))
    t_min = float(_os_landscape.environ.get("LAND_SLICE_T_MIN", "-0.25"))
    t_max = float(_os_landscape.environ.get("LAND_SLICE_T_MAX", "1.25"))
    wmin = float(_os_landscape.environ.get("LAND_WMIN", "1e-4"))
    wmax = float(_os_landscape.environ.get("LAND_WMAX", "5e1"))
    npos = int(_os_landscape.environ.get("LAND_NPOS", "30"))
    nrefine = int(_os_landscape.environ.get("LAND_NREFINE", "3"))
    alpha_margin = float(_os_landscape.environ.get("LAND_STAB_MARGIN", "1e-3"))
    compute_loop = _env_flag("LAND_COMPUTE_LOOP", False)

    _rprint("\n" + "#" * 100)
    _rprint("LANDSCAPE DIAGNOSTICS: 1D PATHS + 2D BASIN SLICE")
    _rprint(f"fixed placement xa/xs = {xa_fixed:+.10f}, {xs_fixed:+.10f}")
    _rprint(f"Hinf search grid: wmin={wmin:g}, wmax={wmax:g}, npos={npos}, nrefine={nrefine}")
    _rprint("K fixed to 0 through all diagnostics")
    _rprint("#" * 100 + "\n")

    cl, b_vec, c_vec = _landscape_build_cldyn(xa_fixed, xs_fixed, r=4)
    controllers = _landscape_builtin_controllers()

    # Convert to theta and evaluate endpoints.
    for c in controllers:
        c["theta"] = _landscape_pack(cl, c["a"], c["n"], 0.0)
        rec = _landscape_eval_theta(
            cl, c["theta"], c["label"], wmin=wmin, wmax=wmax,
            npos=npos, nrefine=nrefine, alpha_margin=alpha_margin,
            loop_metrics=compute_loop, b_vec=b_vec, c_vec=c_vec,
        )
        c["endpoint_eval"] = rec
        _rprint(f"endpoint {c['label']:>20s}: Hinf={rec['Hinf']:.6e}, alpha={rec['alpha']:+.6e}, omega={rec['omega_star']:+.6e}, stable={rec['stable']}")

    label_to_ctrl = {c["label"]: c for c in controllers}
    open_c = label_to_ctrl["open"]
    # Prefer Hinf_good if provided, then BT-LQG, else best known random.
    good_c = label_to_ctrl.get("Hinf_good") or label_to_ctrl.get("BT_LQG_r4") or label_to_ctrl["random_best_noK_249"]
    rand_c = label_to_ctrl.get("random_best_noK_249")

    path_pairs = [(open_c, good_c)]
    if rand_c is not None and rand_c["label"] != good_c["label"]:
        path_pairs.append((open_c, rand_c))
        path_pairs.append((rand_c, good_c))

    fieldnames = ["path", "from", "to", "t", "stable", "Hinf", "alpha", "c_stab", "omega_star", "Ms", "Mt", "KS", "min_return", "eval_error"]
    for c0, c1 in path_pairs:
        rows = []
        ts = _np_landscape.linspace(0.0, 1.0, n_path)
        path_name = f"{c0['label']}_to_{c1['label']}"
        _rprint(f"\n[path] {path_name}: {n_path} point(s)")
        for kk, tt in enumerate(ts):
            theta = (1.0 - tt) * c0["theta"] + tt * c1["theta"]
            rec = _landscape_eval_theta(
                cl, theta, path_name, wmin=wmin, wmax=wmax,
                npos=npos, nrefine=nrefine, alpha_margin=alpha_margin,
                loop_metrics=compute_loop, b_vec=b_vec, c_vec=c_vec,
            )
            rec.update({"path": path_name, "from": c0["label"], "to": c1["label"], "t": float(tt)})
            rows.append(rec)
            _rprint(f"  {kk+1:03d}/{n_path}: t={tt:.3f}, Hinf={rec['Hinf']:.4e}, alpha={rec['alpha']:+.3e}, omega={rec['omega_star']:+.3e}, stable={rec['stable']}")
        csv_name = f"landscape_path_{_landscape_safe_label(path_name)}.csv"
        png_name = f"landscape_path_{_landscape_safe_label(path_name)}.png"
        _landscape_write_rows_csv(csv_name, rows, fieldnames)
        _landscape_plot_path(csv_name, png_name, f"Path {c0['label']} -> {c1['label']}")

    # 2D slice: theta(s,t) = open + s*(good-open) + t*(random-open)
    if rand_c is not None and good_c is not None and rand_c["label"] != good_c["label"]:
        ss = _np_landscape.linspace(s_min, s_max, n_slice)
        tt = _np_landscape.linspace(t_min, t_max, n_slice)
        d_good = good_c["theta"] - open_c["theta"]
        d_rand = rand_c["theta"] - open_c["theta"]
        rows = []
        _rprint(f"\n[2D slice] {n_slice} x {n_slice} = {n_slice*n_slice} point(s)")
        count = 0
        for tv in tt:
            for sv in ss:
                count += 1
                theta = open_c["theta"] + sv * d_good + tv * d_rand
                rec = _landscape_eval_theta(
                    cl, theta, "slice", wmin=wmin, wmax=wmax,
                    npos=npos, nrefine=nrefine, alpha_margin=alpha_margin,
                    loop_metrics=False, b_vec=b_vec, c_vec=c_vec,
                )
                rec.update({"s": float(sv), "t": float(tv)})
                rows.append(rec)
                _rprint(f"  {count:03d}/{n_slice*n_slice}: s={sv:+.2f}, t={tv:+.2f}, Hinf={rec['Hinf']:.3e}, alpha={rec['alpha']:+.2e}, omega={rec['omega_star']:+.2e}")
        slice_fields = ["s", "t", "stable", "Hinf", "alpha", "c_stab", "omega_star", "eval_error"]
        csv_name = "landscape_slice_open_good_random.csv"
        _landscape_write_rows_csv(csv_name, rows, slice_fields)
        _landscape_plot_slice(csv_name, prefix="landscape_slice")
    else:
        _rprint("Skipping 2D slice because good/random controllers are not distinct.")

    _rprint("\nLandscape diagnostics complete. CSV/PNG files written on WORLD rank 0.\n")


if 1==0:
    run_landscape_path_and_slice_diagnostics()
    _sys_landscape.exit(0)

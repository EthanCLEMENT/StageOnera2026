...
def make_pygranso_combined_fn(
    ...
    def combined_fn(X_struct):
        ...
        if verbose and cl.M.comm.rank == 0:
            print(
                f"[oracle] k={k:+.6e}  "
...
#):
    ...
    #def combined_fn(X_struct):
        ...
        #if verbose and cl_mimo.M.comm.rank == 0:
            #print(
                #f"[mimo full + disk] "
...
class ClosedLoopDynamicCompanionSISO:
    ...
    def num_params(self) -> int:
        return 2 * self.r + (1 if self.optimize_mc_gain else 0)
    ...
    def pack_theta(self, a, nvec, mc_gain=None):
        a = np.asarray(a, dtype=float).reshape(self.r)
        nvec = np.asarray(nvec, dtype=float).reshape(self.r)
        if self.optimize_mc_gain:
            if mc_gain is None:
                raise ValueError("mc_gain must be provided when optimize_mc_gain=True")
            return np.concatenate([a, nvec, np.array([float(mc_gain)], dtype=float)])
        return np.concatenate([a, nvec])
    ...
    def dAcl_basis(self, j: int) -> PETSc.Mat:
        ...
        # a_j: affects only last row of L(a)
        if j < self.r:
            dL = np.zeros((self.r, self.r), dtype=np.complex128)
            dL[-1, j] = -1.0
        ...
        # n_j: affects only upper-right block b n^H
        jn = j - self.r
        if jn < self.r:
            dn = np.zeros(self.r, dtype=np.complex128)
            dn[jn] = 1.0
        ...
        if self.optimize_mc_gain and j == 2 * self.r:
...
class ClosedLoopDynamicCompanionSISO:
    ...
    Parameter vector
    ----------------
    theta = [a_0,...,a_{r-1}, n_0,...,n_{r-1}, (mc_gain), K]
    """
    ...
    @property
    def num_params(self) -> int:
        return 2 * self.r + (1 if self.optimize_mc_gain else 0) + 1  # +1 for K
...
for K in [0.0, 0.01, -0.01, 0.05]:
    ...
    for w in [-1.0, -0.8, -0.65, -0.5, 0.5]:
        ...
        if MPI.COMM_WORLD.rank == 0:
            print(f"w={w:+.6f}  dyn={gd:.8e}  stat={gs:.8e}  rel={rel:.3e}")

    ad, _ = cl_dyn.spectral_abscissa_and_grad(theta)
    a_s, _ = cl_stat.spectral_abscissa_and_grad(K)
    if MPI.COMM_WORLD.rank == 0:
        print(f"alpha_dyn  = {ad:.16e}")
        print(f"alpha_stat = {a_s:.16e}")
...
def sample_dynamic_gain_curve(cl_dyn, theta, ws):
    ...
    """
    theta = np.asarray(theta, dtype=float).reshape(-1)
    vals = [cl_dyn.hinf_branch_value(theta, float(w)) for w in ws]
    return np.array(vals, dtype=float)
...
def find_peak_candidate_dynamic(
    ...
    for idx in order[:nref]:
        ...
        w_left  = float(ws[max(0, idx - 1)])
        w_mid   = float(ws[idx])
        w_right = float(ws[min(len(ws) - 1, idx + 1)])
        ...
        local_results.append({
            "grid_index": idx,
            "grid_omega": w_mid,
            "grid_gain": float(vals[idx]),
            "refined_omega": w_loc,
            "refined_gain": g_loc,
            "bracket": (w_left, w_right),
        })
...
def certify_global_peak_disk_dynamic_safe(
    ...
    # low-frequency guard
    for w in [-1e-12, 1e-12, -1e-9, 1e-9, -1e-6, 1e-6]:
        g = cl_dyn.hinf_branch_value(theta, float(w))
        if g > gamma:
            return False, {
                "reason": "low_freq_guard_hit",
                "omega": float(w),
    ...
    while intervals:
        ...
        if midpoint_guard:
            g_mid = cl_dyn.hinf_branch_value(theta, theta_shift)
            if g_mid > gamma:
                return False, {
                    "reason": "midpoint_gain_violation",
        ...
        visited.append((lo, hi, theta_shift, nconv))
...
def make_pygranso_combined_fn_dynamic_Konly(
    ...
    def combined_fn(X_struct):
        ...
        if verbose and cl_dyn.comm.rank == 0:
            print(
...
class ClosedLoopDynamicCompanionSISOStrictlyProper(ClosedLoopDynamicCompanionSISO):
    ...
    Parameter vector
    ----------------
    theta = [a_0,...,a_{r-1}, n_0,...,n_{r-1}, (mc_gain)]
    """
...
theta = np.array([-0.3, 0.0], dtype=float)
...
def make_pygranso_combined_fn_dynamic_strictly_proper(
    ...
    For the strictly proper class, theta has the form
        [a_0,...,a_{r-1}, n_0,...,n_{r-1}, (mc_gain)]
...
a_center = np.array([24.0, 50.0, 35.0, 10.0], dtype=float)
n_center = np.array([0.0, 0.0,0.0,0.0], dtype=float)
theta_center = np.concatenate([a_center, n_center])
theta_scale = np.array([0.0, 0.0,0.0,0.0,2.5e-6,2.5e-6 ,2.5e-6, 2.5e-6], dtype=float)
...
def run_static_for_placement(
    ...
    # freeze a and n, optimize only K
    theta_scale = np.concatenate([
        np.zeros(r, dtype=float),
        np.zeros(r, dtype=float),
        np.array([2.5e-3], dtype=float),
    ])
    ...
    opts.maxit = maxit
    opts.print_frequency = 1
    opts.print_level = 1 if verbose else 0
    opts.quadprog_info_msg = False
    opts.x0 = torch.zeros((cl_dyn.num_params, 1), device=device, dtype=dtype)
...
def run_dynamic_for_order(
    ...
    n_center = np.zeros(r, dtype=float)
    theta_center = np.concatenate([a_center, n_center, np.array([K_seed])])

    # freeze a and K, optimize only n
    theta_scale = np.concatenate([
        np.zeros(r, dtype=float),                 # a frozen
        n_scale * np.ones(r, dtype=float),        # n free
        np.array([0.0], dtype=float),             # K frozen
    ])
...
theta_scale = np.concatenate([
    ...
    np.zeros(r),                 # freeze a
    0.025 * np.ones(r),          # free n, or zeros if testing static
...
def optimize_static_K_for_placement(
    ...
    theta_center = np.concatenate([
        a_center,
        n_center,
        np.array([K_center], dtype=float),
    ])

    theta_scale = np.concatenate([
        np.zeros(r, dtype=float),          # freeze a
        np.zeros(r, dtype=float),          # freeze n
        np.array([K_scale], dtype=float),  # free K only
    ])
...
def optimize_dynamic_n_for_placement(
    ...
    theta_center = np.concatenate([
        a_center,
        n_center,
        np.array([K_seed], dtype=float),
    ])

    theta_scale = np.concatenate([
        np.zeros(r, dtype=float),                 # freeze a
        n_scale * np.ones(r, dtype=float),        # free n
        np.array([0.0], dtype=float),             # freeze K
    ])
...
def optimize_dynamic_K_and_n_for_placement(
    ...
    theta_scale = np.concatenate([
        np.zeros(r, dtype=float),                 # freeze a
        n_scale * np.ones(r, dtype=float),        # free n
        np.array([K_scale], dtype=float),         # free K a little
    ])
...
def make_pygranso_combined_fn_joint_placement_dynamic_n(
    ...
    a_center = np.asarray(a_center, dtype=float).reshape(-1)
    n_center = np.asarray(n_center, dtype=float).reshape(-1)
    ...
    def build_cl_and_theta(xa, xs, nvec, K):
        ...
        theta_ctrl = np.concatenate([
            a_center,
            np.asarray(nvec, dtype=float).reshape(r),
            np.array([float(K)], dtype=float),
        ])
    ...
    def combined_fn(X_struct):
        ...
        g_n_f = g_theta_f[r:2*r]
        g_n_c = g_theta_c[r:2*r]
...
def make_param_sweep_cases(r, a_free_center, n_free_center=None, K_init=0.0):
    ...
    a_free_center = np.asarray(a_free_center, dtype=float).reshape(r)
    n_free_center = np.asarray(n_free_center, dtype=float).reshape(r)
...
def _case_var_slices(case, r):
    # z = [eta_xa, eta_xs, optional z_a, optional z_n, optional eta_K]
    idx = 2
    out = {"xa": 0, "xs": 1, "a": None, "n": None, "K": None}
    if case["free_a"]:
        out["a"] = slice(idx, idx + r)
        idx += r
    if case["free_n"]:
        out["n"] = slice(idx, idx + r)
        idx += r
    if case["free_K"]:
        out["K"] = idx
        idx += 1
...
def _unpack_case_z(z, case, r, xa_bounds, xs_bounds, K_bounds, a_scale, n_scale):
    ...
    a = np.asarray(case["a_center"], dtype=float).reshape(r).copy()
    nvec = np.asarray(case["n_center"], dtype=float).reshape(r).copy()
    K = float(case["K_center"])

    da_dz = np.zeros((r, sl["nvar"]), dtype=float)
    dn_dz = np.zeros((r, sl["nvar"]), dtype=float)
    dK_dz = np.zeros(sl["nvar"], dtype=float)

    if case["free_a"]:
        a_scale_arr = np.asarray(a_scale, dtype=float)
        if a_scale_arr.size == 1:
            a_scale_arr = float(a_scale_arr) * np.ones(r, dtype=float)
        a += a_scale_arr * z[sl["a"]]
        for j in range(r):
            da_dz[j, sl["a"].start + j] = a_scale_arr[j]

    if case["free_n"]:
        n_scale_arr = np.asarray(n_scale, dtype=float)
        if n_scale_arr.size == 1:
            n_scale_arr = float(n_scale_arr) * np.ones(r, dtype=float)
        nvec += n_scale_arr * z[sl["n"]]
        for j in range(r):
            dn_dz[j, sl["n"].start + j] = n_scale_arr[j]
...
def make_pygranso_combined_fn_joint_case(
    ...
    def cache_key(xa, xs, a, nvec, K):
        ...
        return (
            round(float(xa), 12),
            round(float(xs), 12),
            tuple(np.round(np.asarray(a, dtype=float), 12)),
            tuple(np.round(np.asarray(nvec, dtype=float), 12)),
            round(float(K), 12),
        )
    ...
    def eval_physical(xa, xs, a, nvec, K, seed):
        ...
        if case["mode"] == "static":
            ...
            theta = np.concatenate([a, nvec, np.array([K], dtype=float)])
    ...
    def combined_fn(X_struct):
        ...
        if case["mode"] == "static":
            ...
            # Dynamic ctrl gradient is [d/da, d/dn, d/dK].
            df_ctrl = np.asarray(out0["df_ctrl"], dtype=float).reshape(2 * r + 1)
            dc_ctrl = np.asarray(out0["dc_ctrl"], dtype=float).reshape(2 * r + 1)
...
def controller_transfer_response(a, nvec, K, ws):
    ...
    nvec = np.asarray(nvec, dtype=float).reshape(-1)
    r = len(a)
...
def plot_best_closed_loop_gain_curves(
    ...
    for run in runs:
        ...
        if run["case"]["mode"] == "static":
            ...
            theta = np.concatenate([best["a"], best["n"], np.array([best["K"]])])
...
def _robust_loop_metrics_dynamic(A_use, M_use, b_vec, c_vec, a, nvec, K,
    ...
    for w in ws:
        P = _robust_plant_tf_scalar(A_use, M_use, b_vec, c_vec, float(w))
        Hk = _robust_controller_tf_dynamic(a, nvec, K, float(w))
        Lloop = P * Hk
        den = 1.0 - Lloop
        ...
        rows.append((float(w), absS, absT, absKS, abs_return, absL,
                     complex(P), complex(Hk)))
        if absS > Ms:
    ...
    if csv_name is not None and MPI.COMM_WORLD.rank == 0:
...
def _robust_unpack_z(z, r, xa_bounds, xs_bounds, K_bounds,
    ...
    a_center = np.asarray(a_center, dtype=float).reshape(r)
    n_center = np.asarray(n_center, dtype=float).reshape(r)
    a_scale_arr = np.asarray(a_scale, dtype=float)
    n_scale_arr = np.asarray(n_scale, dtype=float)
    if a_scale_arr.size == 1:
        a_scale_arr = float(a_scale_arr) * np.ones(r, dtype=float)
    if n_scale_arr.size == 1:
        n_scale_arr = float(n_scale_arr) * np.ones(r, dtype=float)
    a = a_center + a_scale_arr * z[2:2+r]
    nvec = n_center + n_scale_arr * z[2+r:2+2*r]
...
def _robust_initial_z(r, xa_init, xs_init, K_init, xa_bounds, xs_bounds, K_bounds):
    z0 = np.zeros(2 + 2 * int(r) + 1, dtype=float)
...
def make_pygranso_combined_fn_robust_dynamic_placement(
    ...
    def eval_oracle_from_z(z):
        ...
        theta = np.concatenate([p["a"], p["n"], np.array([p["K"]], dtype=float)])
        out = dynamic_oracle_hard_stability(
            cl_dyn,
            theta=theta,
            stab_margin=stab_margin,
    ...
    def combined_fn(X_struct):
        ...
        df_ctrl = np.asarray(out["df_ctrl"], dtype=float).reshape(2*r + 1)
        dc_ctrl = np.asarray(out["dc_ctrl"], dtype=float).reshape(2*r + 1)
        # theta = [a, n, K]
        df_dz[2:2+r] += df_ctrl[:r] * p["a_scale"]
        df_dz[2+r:2+2*r] += df_ctrl[r:2*r] * p["n_scale"]
        df_dz[-1] += df_ctrl[-1] * p["dK_dz"]
        dcstab_dz[2:2+r] += dc_ctrl[:r] * p["a_scale"]
        dcstab_dz[2+r:2+2*r] += dc_ctrl[r:2*r] * p["n_scale"]
...
# =============================================================================
# Run the robust mixed-sensitivity optimization.
# Tight starting constraints; loosen these if PyGRANSO cannot find feasibility.
# =============================================================================

#robust_run = run_robust_dynamic_placement_pygranso(
    #A=A,
    #M=M,
    #V=V,
    #phi=phi,
...
## Save a fine loop sweep for the best design, if feasible.
#if robust_run["best"] is not None:
    #b = robust_run["best"]
    #b_vec, _ = build_actuator_vector(b["xa"], sigma_gauss, V, phi, bc, is_free)
    #s_vec_best, _ = build_sensor_state_vector(b["xs"], sigma_gauss, V, is_free)
    #c_vec_best = sensor_output_vector(M, s_vec_best)
    #fine_loop = _robust_loop_metrics_dynamic(
        #A, M, b_vec, c_vec_best, b["a"], b["n"], b["K"],
        #wmin=1e-5, wmax=5e1, npos=160,
        #csv_name="robust_dynamic_pygranso_best_loop_fine.csv",
    #)
    #_rprint("Fine loop check for best design:")
